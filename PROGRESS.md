...
- **✂️ /impl-loop 저비용 개선 — agent 5종 return 가이드 구체화 + impl-loop review 출력 재정의 (#446 Step 1)**: jajang 실측 baseline ~280 turn/task 의 post-Agent ~47 turn 누적 주범 중 하나가 review.md 원본 그대로 출력 + agent 본문 장황한 서술. 5 agent (`test-engineer` / `engineer` / `code-validator` / `pr-reviewer` / `module-architect`) 기존 generic return-terseness 가이드 (#440) 옆에 산출물 구조별 구체화 단락 추가 — 테스트 케이스 표 전수 입력 금지 (test-engineer), 파일별 변경 서술 → `M files +X -Y` 통계 (engineer), A/B/C 통과 항목 열거 금지 (code-validator), A~F 체크리스트 본문 재진술 금지 (pr-reviewer), impl 본문 재진술 금지 (module-architect), 공통 워크트리 절대경로 반복 금지. `commands/impl-loop.md` driver 범위 **review 출력 재정의** 절 추가 — 원본 review.md 는 디스크 (`<run_dir>/review.md`) 그대로, 메인 컨텍스트는 5줄 요약 (task 결론 / agent 결과 / finding / PR·이슈 / next). `/impl` 단발 호출은 review.md 원본 그대로 출력 MUST 유지 (`commands/impl.md` 무변경 — 엄격 제약 준수). PR #447.
