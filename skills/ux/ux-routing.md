...
flowchart TB
  ...
  DS2 -->|PASS| PICK
  PICK -->|OK| H([DESIGN_HANDOFF · 종료 → /impl-loop])
...
> 파랑 = 생산 agent · 회색 = 사용자 체크포인트 / 위임. 점선 = escalate. 엣지의 `≤N` = cycle 한도 (§3).
>
...
- **designer 재생성** — 사용자 PICK NG 는 round 한도가 **없다** (사용자 자유 결정, sub_cycle `designer-ROUND-<n>`). cycle 한도(§3)는 *self-check FAIL / 승인 거절* 경로에만 적용.
...
- **화면 전환 / 인터랙션 모호** (ux-architect) → 추측 X. 다중 해석 제시 + 사용자 위임 (PRD 범위 문제면 메인 `/product-plan` 재진입 권고).
