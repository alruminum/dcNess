...
7. **Step 4 — module-architect × K** — Story 단위 + 공통 task 단위 순차 호출. 각 호출 prompt = **슬림 포인터 규약** ([`loop-procedure.md`](../../docs/plugin/loop-procedure.md#호출-prompt-슬림-포인터-규약)): SSOT 포인터(epic architecture/adr/domain-model 경로) + 대상 단위 + 호출별 핵심 제약 + 산출 경로·번호·write 경계. 공통 task 목록·계약 전문을 prompt 에 재기입하지 않는다 — module-architect 가 epic architecture.md 를 자체 read 한다.
   ...
   - **Step 4.1 ~ 4.N (Story 순차)** — **대상 = Story N** 1 개씩 지정해 호출. 산출 = Story 안 task 의 impl 파일 N 개. 각 호출 `READY` 직후 → **commit 4..N+3**
