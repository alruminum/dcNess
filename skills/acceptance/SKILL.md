---
name: acceptance
description: story 또는 epic 구현 완료 후 제품 단위 검수를 수행하는 MVP skill. 사용자가 "/acceptance story", "/acceptance epic", "story 검수", "epic 검수", "AC 기준으로 완료됐는지 봐줘" 등을 말할 때 사용한다. full E2E 검증은 MVP 범위 밖이지만, 핵심 AC가 mock-only green으로만 닫히는 것은 gap으로 분리한다. Lite `/impl` 단발 작업에 자동 강제하지 않는다.
---
