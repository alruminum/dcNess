...
- 구현 (task PR) → `/impl-loop`
- 버그픽스 / qa → `/issue-report`
- 이미 설계 완료된 epic 의 일부 impl 보강 → `/impl-loop` fallback (module-architect 직접)
...
6. **Step 3.5 — architecture-validator 1차** — Placeholder Leak + 공통 SSOT 룰 위반 자동 영역 + **Contract Ledger 충분성** (signature 만 있고 invariant/ordering/error mode/config 빠진 shallow contract 검출) 검증. Cross-Story Interface 는 이 시점 N/A (impl 파일 미작성). → `PASS` → **commit 2** (system-architect 산출물 일괄). **이후 system 문서 freeze** — Step 5 FAIL 이 와도 finding 분류가 `SYSTEM_BOUNDARY` 가 아니면 system-architect 재진입 X (stale 전파는 `CONTRACT_PROPAGATION` sweep).
...
8. **Step 5 — architecture-validator 2차** — Cross-Story Interface 정합성 검증 (모든 impl 완성 후 producer ↔ consumer 시그니처 grep 비교) + Implementation Simulation (대표 impl task 2~3 개 cold-seat 시뮬레이션 — 표시 없는 암묵 gap 조기 포착) + Origin Anchor (PRD `## 수용 기준` AC-NNN ↔ impl REQ `(from AC-NNN)` 인용 커버리지 + 리터럴 self-consistently-wrong 검출 + 참조 실재 + present-vs-예정 + 절차 의미) + **Implementation Detail Leak** (impl 과상세화 — 줄 수·fenced block·금지 토큰) + **Contract Ledger sweep** (`## Contract Ledger` 각 행의 모든 필드 — producer·consumer·invariant·ordering·error mode·config·forbidden alternative — 가 architecture/domain/adr/impl 전반에서 owner(canonical)와 self-consistent 한지 `Grep` 기반 sweep — PASS 게이트) + Placeholder Leak 재검증 (module 단계 새로 생긴 영역). Must finding 마다 분류 동반 — `SYSTEM_BOUNDARY` → system-architect, `CONTRACT_PROPAGATION` → module-architect `mode=contract_sweep` (재설계 X), `TASK_LOCAL` → module-architect 보강 ([`architect-loop-routing.md`](architect-loop-routing.md#finding-분류-라우팅)). → `PASS` → **commit N+4** (검증 결과 메타)
...
> 각 Step 의 agent 결론에 따른 분기·재진입·cycle 한도·escalate = [`architect-loop-routing.md`](architect-loop-routing.md). loop 종료 후 후속(`/impl-loop` 안내 등)도 그 파일.
