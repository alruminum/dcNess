...
- **진행**: 메인이 `docs/prd.md` + (있으면) `docs/tech-review.md` 를 read → 그릴미 패턴 (한 번에 한 질문 / 가설+권장안 제시 / 코드·문서 탐색 우선 / 결정나무 가지치기, [`product-plan-prd-reference.md`](../product-plan/product-plan-prd-reference.md) `## 그릴미 패턴` 차용) 으로 스택 합의. tech-review **축 2 권고** (스펙 강등 / 업그레이드 / 대안 기술) 를 사용자 눈앞에서 채택·미채택 결론까지 도출.
