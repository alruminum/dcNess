...
`architect-loop` (loop 인덱스 = [`docs/plugin/loop-procedure.md`](../../docs/plugin/loop-procedure.md#한눈-인덱스-loop-진입-ssot)). 본 skill 본문 = architect-loop 절차 풀스펙 진본.
...
정상 흐름은 본 skill 본문 + 인용된 docs 섹션 링크 만으로 진행. 본문에 있는 catastrophic / Pre-flight gate / agent boundary 룰이 1차. *룰 모호 / 분기 발생* 시에만 [`architect-loop-routing.md`](architect-loop-routing.md) (라우팅) / `docs/plugin/loop-procedure.md` (절차 mechanics + 한눈 인덱스) / `issue-lifecycle.md` / `git-spec.md` 부분 read (grep + offset/limit). 통째 read 폐기 — 메인 cache_read 기준치 감축.

## 워크트리 (기본 켜짐)

Step 0 진입 시 자동 `EnterWorktree(name="architect-{ts_short}")`. 사용자 발화에 정규식 `워크트리\s*(빼|없|말)` 매치 시에만 건너뜀. 자세히 = [`docs/plugin/loop-procedure.md`](../../docs/plugin/loop-procedure.md#worktree-분기-impl-류-루프-한정).
...
상세 = 본 절차 + [`docs/plugin/loop-procedure.md`](../../docs/plugin/loop-procedure.md#한눈-인덱스-loop-진입-ssot) 인덱스.
...
- loop 인덱스: [`docs/plugin/loop-procedure.md`](../../docs/plugin/loop-procedure.md#한눈-인덱스-loop-진입-ssot) (본 skill 본문 = architect-loop 절차 풀스펙)
