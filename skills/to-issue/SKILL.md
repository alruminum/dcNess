...
보드 좌표가 있으면(또는 위에서 셋업했으면) 생성된 issue 를 Project 보드에 등록한다. `register-issue` 가 item 추가(없으면 add, 멱등) + `Status=Todo` + 선택한 `IssueType` + 선택한 `Priority` 설정 + drift 사후검증을 한 번에 처리한다. Project field 와 option id 는 스크립트가 `gh project field-list` 로 조회한 실제 값만 사용한다.
