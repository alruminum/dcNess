...
4. **산출물 경로 반환** — `PASS` 시 작성된 `docs/compact-plans/<slug>.md` 경로를 호출 측(`/impl`)에 돌려준다. 이 경로가 후속 구현의 **engineer 게이트 사전 조건(설계 산출물 실존)** 증거다.

## 호출 측(`/impl`)으로의 복귀 — engineer 게이트 사전 조건

본 스킬이 compact plan 을 산출하면 `/impl` 은 그 경로를 들고 **Standard 구현 경로로 (재)진입**한다. impl 은 같은 run 에서 설계를 생성하지 않으므로, 진입 시 `begin-run impl --design-doc docs/compact-plans/<slug>.md` 로 산출물을 run 에 기록한다 — engineer 게이트가 그 실존을 설계 산출물 사전 조건 증거로 인정한다([`hooks.md` engineer gate](../../docs/plugin/hooks.md#catastrophic-gatesh)).

compact plan 이 같은 세션에서 막 산출됐든, 이전에 머지된 뒤 별도 run 으로 진입하든, 게이트가 보는 것은 "설계 산출물의 실존" 하나다 — `--design-doc` 으로 기록한 설계도 경로다. 이것이 #702 가 게이트에 적용한 원리(impl 은 설계 문서 유무만 보면 된다)를 #711 이 완성한 형태다: impl 은 설계를 만들지 않고 받은 설계도로 구현만 하며, Standard 구현 경로는 항상 `--design-doc` 기록으로 진입한다.
...
- 공개 진입점 계약: [`positioning.md`](../../docs/plugin/positioning.md)
