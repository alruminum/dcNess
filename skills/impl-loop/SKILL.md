...
- **frontmatter 우선 (진본, #703)**: impl 문서 frontmatter 에 `risk` / `engine` 이 있으면 **추론하지 말고 그 값을 쓴다** — `engine: 4agent` → 풀 4-agent · `engine: 2agent` → build-worker, `risk: high` → 풀 4-agent 승격(아래 자동 승격과 동치). 고위험 판정은 설계자(module-architect)가 task 를 자르는 시점에 이미 알고 frontmatter 에 박은 값이라 진입마다 재추론할 필요가 없다. frontmatter 에 risk/engine 필드가 **없을 때만** 아래 디폴트·추론 fallback 으로 떨어진다(하위호환).
...
각 task 는 dry preview 단계에서 `risk` / `engine` / `reason` 을 남긴다. **frontmatter 에 `risk`/`engine`/`risk_reason` 이 있으면 추론하지 말고 그 값을 그대로 옮긴다 (#703)**: `risk` ∈ `normal`/`high`/`low`, `engine` = `2agent`(build-worker) / `4agent`(풀 4-agent), `reason` = frontmatter `risk_reason`. frontmatter 에 risk 필드가 **없는 task 만** 메인이 본문에서 추론한다 — 그 경우 `risk` 는 `normal`/`high`, `reason` 은 고위험 trigger 가 없으면 `고위험 trigger 없음`, 고위험이면 `외부 HTTP`/`URL 파싱`/`auth`/`PII`/`도메인 invariant` 처럼 task 본문에서 확인한 근거를 적는다. 어느 경로든 `reason` 은 비워 두지 않는다 (frontmatter `risk_reason` 또는 추론 근거 중 하나는 항상 채운다). `risk: high` row 의 기본 `engine` 은 `4agent`(풀 4-agent) 이며, chain 전체 기본이 build-worker 여도 해당 row 만 승격한다. (verify-only task 는 risk 와 별개로 `task_type` 으로 판정 — 위 `### verify-only task`.)
