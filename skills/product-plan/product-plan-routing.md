...
>
> `/tech-review` 는 본 skill *종료 후* 단계다. 외부 의존 0 개 PRD 면 skip 하고 바로 `/architect-loop`. **`/architect-loop` 진입 후 `/tech-review` 재호출은 금지** (단방향 catastrophic, [escalate · 재진입 · 단방향 catastrophic](#escalate-재진입-단방향-catastrophic)).
