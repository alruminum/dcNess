...
| Deep | high-risk trigger 있거나 이미 deep impl task 파일이 있음 | `/product-plan` / `/architect-loop` / `/impl-loop` 로 위임 |
...
- **Deep lane**: 본 skill 이 직접 deep loop 를 재서술하지 않는다. deep impl task 파일이 있으면 `/impl-loop <task>` 로 위임하고, 없으면 `/product-plan` 또는 `/architect-loop` 선행을 권고한다.
...
3. 목표/범위/성공 기준이 모호한가? → 사용자에게 명확화 또는 `/product-plan`
...
- PRD/stories 가 없다 → `/product-plan` 부터 시작한다.
- PRD/stories 는 있으나 architecture/impl task 가 없다 → `/architect-loop` 로 설계한다.
