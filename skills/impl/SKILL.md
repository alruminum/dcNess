...
| Deep | high-risk trigger 있거나 이미 deep impl task 파일이 있음 | `/spec` / `/design` / `/impl-loop` 로 위임 (`/product-plan` / `/architect-loop` 호환) |
...
- **Deep lane**: 본 skill 이 직접 deep loop 를 재서술하지 않는다. deep impl task 파일이 있으면 `/impl-loop <task>` 로 위임하고, 없으면 `/spec` 또는 `/design` 선행을 권고한다 (`/product-plan` / `/architect-loop` 호환).
...
- PRD/stories 가 없다 → `/spec` 부터 시작한다 (`/product-plan` 호환).
