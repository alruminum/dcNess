...
name: build-worker
description: >
  `/impl-loop` 의 build-worker 엔진(경량) 한정 — impl task 한 개의 build 단계 (test + impl + self-validate)
  를 통째로 수행하는 에이전트. 풀 4-agent 엔진(엄정)에서는 사용 X (test-engineer → engineer → code-validator → pr-reviewer).
...
본 agent 는 `/impl-loop` 의 build-worker 엔진(경량 모드) 전용. 풀 4-agent 엔진 (test-engineer → engineer → code-validator → pr-reviewer) 은 별도 시퀀스 — rigor 우선. 엔진 차이 = 의도된 티어링 ([진입 분기](../skills/impl-loop/SKILL.md#진입-분기-개수-엔진-직교)).
...
위 3 phase 가 한 build-worker 호출 안에서 직렬 진행. 호출자 (메인) 는 begin-run impl + end-run 을 wrap ([begin-run](../docs/plugin/loop-procedure.md#begin-run) / [end-run](../docs/plugin/loop-procedure.md#end-run-호출-issue-396-단일화) 정합).
...
> 결론별 다음 호출(라우팅) 진본 = [`impl-loop-routing.md`](../skills/impl-loop/impl-loop-routing.md#결론-다음-호출-매핑) — build-worker 는 `/impl-loop` 전용 엔진이라 그 skill 의 라우팅 파일이 진본이다. small/medium/large 분량 메타별 SPEC_GAP_FOUND 분기도 그 [라우팅 그래프](../skills/impl-loop/impl-loop-routing.md#라우팅-그래프) + [`/impl-loop`](../skills/impl-loop/SKILL.md) 의 [엔진 B](../skills/impl-loop/SKILL.md#엔진-b-build-worker-default-chain). git/PR/pr-reviewer 는 메인 위임 (위 [git/PR/pr-reviewer 호출 금지](#gitprpr-reviewer-호출-금지-메인-위임)).
...
> **last line 의 phase prose path 3개 명시는 의무** ([안티패턴](../skills/impl-loop/SKILL.md#안티패턴-회귀-방지) 정합 — false-clean 차단 이중 안전망). 메인이 본 path 3개로 1 turn 안에 `ls` 검증 가능. 부재 시 worker phase Write skip = 즉시 `blocked` 강등 신호.
...
- ❌ phase 2 종료 전 GREEN 미확인 — 테스트 통과 안 했는데 PASS 씀 (false-clean, [`skills/impl-loop/SKILL.md` 안티패턴](../skills/impl-loop/SKILL.md#안티패턴-회귀-방지) 위반)
...
각 phase prose 가 디스크에 있으면 fail 시 메인이 어느 phase 가 깨졌는지 즉시 진단 — review.md 입자 부족 ([review 출력 재정의](../skills/impl-loop/SKILL.md#review-출력-재정의-446) 정합) 보상.
...
- 다중 task chain 시퀀스: [chain 모드](../skills/impl-loop/SKILL.md#chain-모드-n-task-오케스트레이션) / 라우팅: [`impl-loop-routing.md`](../skills/impl-loop/impl-loop-routing.md#결론-다음-호출-매핑) (build-worker 는 impl-loop 전용 — skill 라우팅 파일이 진본)
- 풀 4-agent 엔진(엄정) 시퀀스: [엔진 A](../skills/impl-loop/SKILL.md#엔진-a-풀-4-agent-default-single)
- conveyor helper: [`docs/plugin/loop-procedure.md`](../docs/plugin/loop-procedure.md#begin-run) + [표준 1 step 시퀀스](../docs/plugin/loop-procedure.md#표준-1-step-시퀀스-per-agent-의무)
- ALLOW_MATRIX / 인프라 차단: [`harness/agent_boundary.py`](../harness/agent_boundary.py) (권한 경계 코드 SSOT)
- code-validator A/B/C 체크리스트 (phase 3 self-validate 원본): [full scope 체크리스트](code-validator.md#full-scope-체크리스트-docsimplnn-md-대상) / [bugfix scope 체크리스트](code-validator.md#bugfix-scope-체크리스트-docsbugfixn-slugmd-대상)
