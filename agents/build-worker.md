...
- **SPEC_GAP_FOUND** — phase 1 또는 phase 2 에서 impl 계획 불충분. 본문에 누락 항목 + **추정 분량 메타 (small / medium / large) 1줄 의무**:
  - `small` = 1 enum 값 / 1 필드 / 1 메서드 시그니처 추가 — 메인이 직접 Edit 진행 권고 (cycle 카운트 불포함).
  - `medium / large` = multiple field / 새 module / 도메인 모델 변경 — module-architect (보강) 권고. 분기 룰 = [`commands/impl-loop.md`](../commands/impl-loop.md) §절차 의 SPEC_GAP_FOUND 분기 정합.
