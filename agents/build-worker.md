...
본 agent 는 `/impl-loop` 의 build-worker 엔진(경량 모드) 전용. 풀 4-agent 엔진 (test-engineer → engineer → code-validator → pr-reviewer) 은 별도 시퀀스 — rigor 우선. 엔진 차이 = 의도된 티어링 ([`/impl-loop`](../skills/impl-loop/SKILL.md) §진입 분기).
...
> 결론별 다음 호출(라우팅) 진본 = [`impl-loop-routing.md`](../skills/impl-loop/impl-loop-routing.md) §2 — build-worker 는 `/impl-loop` 전용 엔진이라 그 skill 의 라우팅 파일이 진본이다. small/medium/large 분량 메타별 SPEC_GAP_FOUND 분기도 §2 + [`/impl-loop`](../skills/impl-loop/SKILL.md) §엔진 B. git/PR/pr-reviewer 는 메인 위임 (위 §"git/PR/pr-reviewer 호출 금지").
...
> **last line 의 phase prose path 3개 명시는 의무** ([`/impl-loop`](../skills/impl-loop/SKILL.md) §안티패턴 정합 — false-clean 차단 이중 안전망). 메인이 본 path 3개로 1 turn 안에 `ls` 검증 가능. 부재 시 worker phase Write skip = 즉시 `blocked` 강등 신호.
...
- ❌ phase 2 종료 전 GREEN 미확인 — 테스트 통과 안 했는데 PASS 씀 (false-clean, `skills/impl-loop/SKILL.md §false-clean 차단` 위반)
