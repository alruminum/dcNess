...
- **권한/툴 부족 시 사용자에게 명시 요청 (DCN-CHG-20260430-18, 공통 지침)** — 검증에 필요한 도구·권한·정보 부족 시 *추측 verdict X*. 메인 Claude 에게 (a) 무엇이 부족 (b) 왜 필요 (c) 어떻게 얻는지 명시 요청. 예: "외부 API 응답 검증 위해 호출자가 결과 prose 로 전달 필요". (Karpathy 원칙 1 정합)
