...
- **`docs/domain-model.md` 권한 read** — 도메인 invariant 위반 = 보안 이슈 가능 (예: "음수 amount 결제 불가" invariant 위반 → 무료 결제 우회). on-demand 참조. 수정 금지.
- **권한/툴 부족 시 사용자에게 명시 요청** — 보안 검토에 필요한 도구·권한·정보 부족 시 *추측 finding X*. 메인 Claude 에게 명시 요청 후 진행. 예: "외부 API 인증 흐름 확인 위해 vendor 문서 fetch 필요". (Karpathy 원칙 1 정합)
