...
> 결론별 다음 호출(라우팅) 진본 = [`docs/plugin/orchestration.md`](../docs/plugin/orchestration.md) §3.1 routing 한눈표. 위 escalate-케이스는 *언제 escalate 하는가* + catastrophic 제약 영역 (ESCALATE / NEW_DEP_ESCALATE 처리 상세 = orchestration §3.3 + §2.1.4).
...
- 시퀀스 / 핸드오프 / 권한: [`docs/plugin/orchestration.md`](../docs/plugin/orchestration.md)
