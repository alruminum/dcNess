...
- **frontmatter** — `depth: simple|std|deep`, `design: required` (스크린샷 변경 시만), **`story: <N>`** (어떤 Story 종속 — 단일 또는 `공통`), **`task_index: <i>/<total>`** (대응 Story 안 본 task 의 순번 / Story 의 총 task 수; `(공통)` task 는 `—`). `task_index` 는 호출자 (메인 Claude) 가 prompt 에 써주는 값을 *그대로* frontmatter 에 옮김 — impl-task-loop PR body Closes/Part of 판정 입력 ([`docs/plugin/issue-lifecycle.md`](../docs/plugin/issue-lifecycle.md) §1.4)
