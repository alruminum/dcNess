...
> 결론별 다음 호출(라우팅) 진본 = [`docs/plugin/handoff-matrix.md`](../docs/plugin/handoff-matrix.md) §1.0 routing 한눈표. 위 escalate-케이스는 *언제 escalate 하는가* + catastrophic 제약 영역 (ESCALATE / NEW_DEP_ESCALATE 처리 상세 = handoff-matrix.md §3 + orchestration §2.1.4).
