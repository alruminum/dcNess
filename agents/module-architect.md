...
- **계약 전파 sweep 케이스** — 대상 epic 경로 + `mode=contract_sweep` + 확정 canonical 계약 값 + sweep 키워드 목록 (architecture-validator 가 `CONTRACT_PROPAGATION` finding 으로 넘긴 것). 자세히 = [`## 계약 전파 sweep`](#계약-전파-sweep).

호출자가 케이스 명시 안 해도 prompt 본문으로 자율 판단.

## 권한 경계

- **Write 허용**: `docs/milestones/**/impl/**` + `docs/milestones/**/architecture.md` + `docs/milestones/**/domain-model.md` + `docs/milestones/**/adr.md` + `docs/bugfix/**`. **`mode=contract_sweep` 한정 확장**: stale 계약 사본 동기화를 위해 root `docs/architecture.md` / root `docs/adr.md` 포함 `docs/**` 의 계약 줄 patch 허용 (sync only — 재설계 X, system freeze 와 양립). `docs/**` 밖(`skills/` · `scripts/` · helper)은 patch 금지 → 보고+escalate.
...
- **구체화** — impl 파일 본문 *재진술 금지*. 작성 / 수정한 섹션 라벨 (예: "## Contract / ## 인터페이스 / ## 수용 기준") + 변경 의도 1-2 문장. 의사코드 · Props 타입 · 시그니처 본문 전수 echo 금지. 워크트리 절대경로 반복 echo 금지 — 처음 1회만 박고 이후 상대경로 또는 생략.
...
- **frontmatter** — `depth: simple|std|deep`, `design: required` (스크린샷 변경 시만), `story: <N>` (어떤 Story 종속 — 단일 또는 `공통`), `task_index: <i>/<total>` (대응 Story 안 본 task 의 순번 / Story 의 총 task 수; `공통` task 는 `—`). cross-task 계약을 produce/consume 하면 `contract: produces [<id>...] / consumes [<id>...]` (Contract Ledger 항목 id; 없으면 생략).
...
- **`## Scope` 섹션 (필수, 원칙 1)** — 본 task 가 어떤 레이어 / 모듈 다루는지 명시. **`### 수정 금지` (Do Not Modify)** 하위에 *건드리면 안 되는 파일 / 경계* 를 이유와 함께 명시 (다른 레이어 손대지 X).
- **`## Contract` 섹션 (cross-task 계약 produce/consume 시 필수)** — 그 task 가 다루는 계약을 표로: `owner / producer / consumer / invariant / ordering / error mode / config / forbidden alternative`. epic architecture.md 의 `## Contract Ledger` 항목과 1:1 대응. 시그니처가 아니라 *caller 가 올바르게 쓰기 위한 완전한 의미* 를 담는다. cross-task 계약 없으면 생략.
- **`## 인터페이스` 섹션 (필수, 원칙 4)** — **public class / function / protocol 시그니처만**. private helper 이름·본문 명시 금지 (engineer 재량). 내부 구현 X. 도메인 핵심 규칙 (멱등성 / 보안 invariant / 데이터 무결성) 은 `## Contract` 의 invariant 열 또는 본 섹션 주석으로.
- **`## 핵심 로직` 섹션 (기본 금지)** — 원칙적으로 쓰지 않는다. 시그니처 + Contract 로 부족하고 *진짜 알고리즘* (파서 / 상태머신 / 정렬 / 매칭) 이 있을 때만 예외 허용: **≤ 8 줄 + public behavior 흐름만 + private helper / 구체 loop body 금지**. 8 줄 초과 · 구현 선점 시 architecture-validator 영역 6 (Implementation Detail Leak) FAIL.
...
**Implementation Detail Leak 금지 패턴** (contract 는 자세히, implementation 은 얕게):

- private helper 이름 강제 금지 (`_grade_scenes` 같은 이름은 예시도 지양). 테스트 함수명 강제 금지 — *테스트할 behavior* 만 지정.
- code block 안에 list comprehension / 구체 loop body / try-except 의사코드 / `model_dump` · `append` · `result = []` · `for x in xs` 같은 구현 detail 금지.
- "기존 / 변경 후" 코드 블록으로 내부 구현 흐름 지정 금지. composition 실제 snippet 은 5 줄 이하 또는 signature-level 만.
- 이유: 시그니처 하나 바뀌면 의사코드 · 테스트명 · snippet 까지 전부 stale 이 돼 drift 표면이 커진다. engineer 가 구현 결정을 할 여지를 문서가 선점하지 않는다.

**depth 기준**: simple (기존 코드 구조 수정) / std (새 로직 구조 신설) / deep (보안 민감 — auth · 결제 · 암호화).

`deep` depth 일 때 **보안 invariant 명시 의무** — impl `## Contract` 의 invariant 열 또는 `## 인터페이스` 주석에 도메인 invariant (예: `amount > 0`, `userId == session.userId`, `token.exp > now`) 를 코드 가드 위치와 함께 작성. 단순 "검증해야 한다" 자연어 금지. (`## 핵심 로직` 은 기본 금지이므로 invariant 의 1차 거처는 `## Contract`.)
...
## 계약 생명주기 — CONTRACT_AMENDMENT (public contract 변경 시 행동 의무)

module 단계에서 public contract 를 *바꾸게* 되면 (시그니처 / invariant / ordering / error mode / config / consumer / forbidden alternative — 어느 Ledger 필드든 변경), **조용히 자기 task 만 고치지 않는다.** 다음을 명시한다 (자연어 행동 — 강제 enum / marker 아님):

1. **Contract Ledger 갱신** — epic architecture.md 의 `## Contract Ledger` 해당 행을 변경 후 값으로 갱신. 변경 전 / 후, 영향 받는 consumer 목록, forbidden alternative sweep 키워드를 return prose 에 명시.
...
> `mode=contract_sweep` 케이스. architecture-validator 가 `CONTRACT_PROPAGATION` finding (결정은 맞는데 stale 사본이 문서마다 남음) 으로 넘긴 것을 받아 **targeted 동기화** 만 한다. **재설계 아님 — system 문서 freeze 유지.**

입력: 확정 canonical 계약 값 + sweep 키워드 목록.
...
architecture-validator FAIL 로 재진입 시, finding 을 "그 점만" 고치지 말 것. finding 이 설계 전제·직교 조합의 *증상*인지 먼저 본다 — **같은 영역 finding 이 2회+ 반복되면 점 패치 신호**이므로 접근 자체를 재설계한다 (한 번의 근본 재설계 < N 번 점 패치 + N 번 재검증). 진본 = [`loop-procedure.md` finding 수용 원칙](../docs/plugin/loop-procedure.md#finding-수용-원칙-점-패치-금지-근본-수정).
...
- **Contract 정합성** — cross-task 계약을 produce/consume 하면 impl `## Contract` 가 epic architecture.md `## Contract Ledger` 항목과 일치 (owner / producer / consumer / invariant / ordering / error mode / config / forbidden alternative — 모든 Ledger 필드). 계약을 바꿨으면 Ledger 갱신 + CONTRACT_AMENDMENT 보고 (또는 "Ledger 변경 없음" 명시).
- **Implementation Detail Leak 없음** — `## 핵심 로직` 이 ≤ 8 줄이고 private helper 이름 / 구체 loop body / 의사코드로 구현을 선점하지 않음. 시그니처 + Contract 로 contract 는 충분, implementation 은 engineer 재량으로 남김.
...
- (계약 변경 시) CONTRACT_AMENDMENT — 갱신한 Contract Ledger 항목 + 변경 전 / 후 + 영향 consumer (또는 "Ledger 변경 없음")
- (sweep 케이스) sweep 결과 — 검색 키워드 / stale 위치 (file:line) / patch 줄 수 / 남은 미해결
- (보강 케이스) 해소된 갭 목록
