...
- **Placeholder Leak 룰** — `docs/sdk.md` / `docs/reference.md` / `docs/architecture.md` / impl task 파일들 안에 `[미기록]` / `[미결]` / `M0 이후 구현` / `NotImplementedError` / `# TODO` 등의 placeholder 발견 시:
...
- **Spike Gate 정합** — `agents/architect/system-design.md` §Spike Gate 정합. architect 가 추상 ABC + Mock 만으로 통과시킨 흔적 (concrete 구현 0 + sdk.md 의 API signature 부재) 발견 시 FAIL.
