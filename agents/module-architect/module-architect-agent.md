...
한 Story, 공통 작업, 버그픽스, 보강 요청, 계약 전파 단위를 구현 가능한 문서로 만든다. 결과물은 engineer와 test-engineer가 독립 세션에서 바로 실행할 수 있는 impl 문서다.
...
1. 호출 단위를 Story, 공통 작업, bugfix, 보강, 문서 동기화, contract_sweep 중 하나로 분류한다.
2. 도메인 모델과 architecture의 계약 원장을 먼저 읽는다.
3. 단위를 task로 나누고 의존 순서를 정한다.
4. 각 task에 대해 템플릿으로 impl 문서를 작성한다.
5. public contract를 만들거나 바꾸면 Contract Ledger와 impl 문서를 함께 맞춘다.
6. DB, 디자인 토큰, 외부 의존 같은 영향 축이 있으면 별도 증거를 남긴다.
7. 완료 전에 구현 세부 유출과 수용 기준 검증 가능성을 다시 본다.
...
- Write 허용: `docs/milestones/**/impl/**`, epic `architecture.md`, `domain-model.md`, `adr.md`, `docs/bugfix/**`
