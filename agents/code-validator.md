...
- **ESCALATE** — 사용자 위임 (재시도 한도 초과, spec 부재, 기타 진행 불가). 본문에 *사유* 명시 (예: "impl 계획 파일 + 대체 소스 모두 부재", "재시도 3회 후에도 동일 항목 FAIL"). 메인 Claude 가 사유 보고 module-architect (보강 케이스) 호출 등 라우팅 결정.
