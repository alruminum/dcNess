...
prose 마지막 단락에 *어떤 결과로 끝났는지 (+ 사유)* 자기 언어로 명시. 결론 vocabulary:

- **PASS** — 검증 통과
- **FAIL** — 검증 실패 (불일치 항목 명시)
- **ESCALATE** — 진행 불가 (재시도 한도 초과, spec 부재 등). 본문에 *사유* 명시 (예: "impl 계획 파일 + 대체 소스 모두 부재", "재시도 3회 후에도 동일 항목 FAIL").
...
**산출물 경로 백틱 의무 (#457 처방 2-(1))**: return prose 안 모든 산출물 언급 (impl.md / 코드 파일 / SPEC_GAP 위치 / 디스크 review.md) 은 *백틱으로 감싼 경로* 형태로 쓴다. 메인 Claude 가 그대로 사용자 응답에 echo 하므로 사용자가 한 번 클릭으로 도달. 파일명만 / "위 impl" 같은 모호 참조 / 백틱 누락 형태 금지.

**구체화 (#446)**: PASS 시 full scope 의 A/B/C (또는 bugfix scope 의 A/B) 통과 항목 **열거 금지** — "A/B/C 통과 (full)" 또는 "A/B 통과 (bugfix)" 1줄로 종결. FAIL 시만 Fail Items 명시 (계층 라벨 + 파일:라인 + 문제 1-2 문장). impl 계획 본문 재진술 금지 — 항목 라벨 + REQ-NNN ID 만 인용, 의사코드·인터페이스 본문 다시 쓰지 X. 워크트리 절대경로 (`/Users/.../worktrees/...`) 반복 echo 금지 — 처음 1회만 박고 이후 상대경로 또는 생략.
...
- **design.md 토큰 참조 무결성** (design.md 존재 시): `{colors.X}` / `{typography.X}` / `{rounded.X}` / `{spacing.X}` / `{components.X}` 등 참조가 frontmatter 에 실재하는지 확인 ([`docs/design.md` 토큰 참조 무결성 검증](../docs/plugin/design.md#토큰-참조-무결성-검증) 정합). 미실재 참조 = FAIL
...
- 가정 명시 — "spec 의 X 항목을 Y 로 해석해 검증" prose 에 씀
