...
- PRODUCT_PLAN_CHANGE: 기존 `docs/prd.md` 경로, 변경 요청 내용

## 권한 경계 (catastrophic)

- **Write 허용**: `docs/prd.md`, `stories.md`
- **src/ 읽기 금지**: 코드 읽으면 구현 수준으로 내려감 → 기획자 추상화 깨짐
- **docs/architecture.md 읽기 금지**: 기술 세부가 요구사항 왜곡 (architect 단독 소유)
...
`docs/prd.md` + `stories.md` Write 완료 후 [`docs/plugin/issue-lifecycle.md`](../docs/plugin/issue-lifecycle.md) §1 에 따라 epic + story 이슈 연속 생성:
...
6. story 이슈 N개 생성 완료 후 prose 마지막에 sub-issue 연결용 정보 박음 (메인 Claude 가 issue-lifecycle §1.3.1 따라 일괄 호출):
   - epic 이슈 number
   - 각 story 이슈의 database id (`mcp__github__create_issue` 응답의 `.id` 필드, `.number` 아님)
   - product-planner 자신은 `gh api` 호출 X — 정보만 emit

**레이블 / 마일스톤 형식**: issue-lifecycle.md §1.2~§1.3 단일 SSOT. 본 문서엔 재기술 X.

### PRODUCT_PLAN_READY 산출물 (정보 의무, 형식 자유)

`docs/prd.md` 에 다음 정보 포함:
- 서비스 개요 (목적·타겟·핵심 가치)
...
이미 `docs/prd.md` 가 있는 상태에서 요구사항 변경 시. 트리거: 유저가 "이거 바꾸고 싶어", "기능 추가할게", "BM 변경됐어".
