...
권장 결론 표현:
- 시안 준비 완료 → 사용자 직접 확인 (Pencil 캔버스 또는 `design-variants/<screen>-v<N>.html`). 권장: "DESIGN_READY_FOR_REVIEW".
- 시안 생성 불가 (외부 의존 부재 / 컨텍스트 모호 / 권한 부족) → 사용자 위임. "DESIGN_LOOP_ESCALATE".
...
prose 결론 단락에 컨셉 + Pencil 프레임 ID + 스크린샷 경로 + 색상/서체/애니메이션 스펙. 결론 enum `DESIGN_READY_FOR_REVIEW`.
...
prose 결론 단락에 컨셉 + 생성된 `.html` 파일 경로 + canvas.html iframe 추가 위치 + 색상/서체/애니메이션 스펙 + 사용자에게 안내할 node-id 목록. 결론 enum `DESIGN_READY_FOR_REVIEW`.
...
- 컨텍스트 모호 / 새 방향 떠올리기 어려움 → `DESIGN_LOOP_ESCALATE`
