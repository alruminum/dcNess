...
> ⚠️ extended thinking 본문 드래프트 금지. thinking = 의사결정 분기만. 본문·도메인 표·모듈 분해 = thinking 종료 *후* `Write` 입력 안에서만. 위반 시 THINKING_LOOP 회귀 (실측 6분 stall 사례).
...
기술 에픽 케이스 (기술 부채 / 인프라 / 리팩토링 / 아키텍처 변경) — 기능 에픽이 아닌 *기술* 목적 호출 시 추가로: 다음 에픽 번호 확인 (GitHub Issues milestone=Epics) → 에픽 + 스토리 이슈 등록 (`docs/plugin/issue-lifecycle.md` §1.2~§1.3) → 프로젝트 `CLAUDE.md` 에픽 목록 갱신. 결론 prose 에 epic + story `.id` 써서 메인이 sub-issue 연결 일괄 처리.
...
- **기술 스택 ADR** (`docs/adr.md`) — 영역별 선택 + 이유 + 버린 대안 + 이유. 상태: `Proposed` / `Accepted` / `Superseded by ADR-NN`. **갱신 의무**: 기술 스택 / 핵심 설계 결정 신규·변경 시 `docs/adr.md` 추가. *부재 시 silent skip* (UI 없거나 ADR 도입 안 한 프로젝트). impl 파일의 `## 사전 준비` 섹션이 본 ADR 을 read 의무로 씀 (`agents/module-architect.md` §impl 파일 7 원칙).

  - **Migration / 외부 의존 갈아끼움 ADR self-check** — ADR 작성 시 "버린 대안" 표가 4 카테고리 (A 단일 라이브러리 / B 라이브러리 합성 / C 자체 native 구현 / D 요구사항 강등 + UX 보강) 를 *모두 자문* 했는지 1회 점검. 카테고리 누락 발견 시 자체 enumerate 또는 명시적 "N/A — 이유" 씀. 형식 강제 X (자연어 의무) — `agents/plan-reviewer.md` §8.3.2 와 같은 카테고리, 책임 영역만 다름 (plan-reviewer = PRD 단계 / system-architect = ADR 단계). 근거: 실측 회귀 사례 — "버린 대안" 표가 외부 라이브러리 fork 변종만 열거하면 합성/자체/강등 path 검토 0건이 되어 system-architect 가 받은 framing 그대로 ADR 작성하는 패턴.
...
규칙:
- `NN` = epic 내 독립 순번
- `task_index = i/total` — 대응 Story 안에서 본 task 의 순번 / Story 의 총 task 수. `(공통)` 행은 `—`. **impl-task-loop PR body `Closes`/`Part of` 판정 입력** ([`docs/plugin/git-spec.md`](../docs/plugin/git-spec.md) §8 PR 트레일러)
...
산출 위치: `docs/architecture.md` `## impl 목차` 섹션. 메인 Claude 가 표 행 K 개를 순회하며 module-architect 1번씩 호출. 각 호출 prompt 에 `task_index` 써서 module-architect 가 impl 파일 frontmatter 에 박도록 지시.
...
- ✅ 쓴다: **큰 모듈의 흐름** (모듈 목록 / 의존 그래프 / 데이터 흐름) + **인터페이스 정의** (함수/클래스 시그니처) + **NFR 목표** + **impl 목차 표** + **기술 스택 ADR 참조**
...
> 사례: 외부 검증 누락 실측 — 후보 N개 비교를 "M0" 로 미루고 ABC + Mock 만으로 PASS 통과 → 다수 기능 구현 후 *핵심 가치 검증 불가* 발견. "미래의 약속은 검증이 아니다."
