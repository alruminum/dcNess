...
- **기술 스택 ADR** — 영역별 선택 + 이유 + 버린 대안 + 이유. 상태: `Proposed` / `Accepted` / `Superseded by ADR-NN`
...
| NN | impl 파일명 | 대응 Story | depth | 의존 | 1줄 요약 |
|----|-------------|-----------|-------|------|---------|
...
산출 위치: `docs/architecture.md` `## impl 목차` 섹션. 메인 Claude 가 표 행 N 개를 순회하며 module-architect 1번씩 호출.
...
> 추상 인터페이스 (ABC / Protocol) + Mock 구현만으로 READY 통과 금지. PRD Must 기능의 functional core 직결되는 외부 의존 (모델·SDK·API) 은 시스템 설계 *전* 또는 *중* 에 *실제 1개 spike* 로 검증.
...
**Spike FAIL** → 조용히 다른 후보로 진행 X. 결론 `ESCALATE` + (실측 결과 / 깨진 가정 / 옵션 권고) 명시. product-planner 재호출 권고.

> 사례: jajang 2026-04 — 후보 4개 (OpenVoice V2 / F5-TTS / RVC / CosyVoice) 비교를 "M0" 로 미루고 ABC + Mock 으로 READY 통과 → 14 기능 구현 후 *허밍 합성 불가* 발견. "미래의 약속은 검증이 아니다." 자세한 경위는 `docs/archive/spike-gate-jajang.md`.
...
- **Spike Gate 결과** — PRD Must 직결 외부 의존 list + 각 spike 통과 여부 + 검증된 SDK/모델 명. spike 미실행 시 READY 출력 X
