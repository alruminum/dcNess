...
| 변경 유형 | 동기화 대상 |
|---|---|
| **도메인 모델 변경** (entity/VO/aggregate/invariant) | **`docs/domain-model.md` (architect 단독 수정 권한)** |
| 시스템 구조·모듈 의존성 변경 | `docs/architecture.md` + 분리된 detail 파일 |
| 게임 로직·알고리즘·수치 변경 | `docs/game-logic.md` (또는 프로젝트 해당 문서) |
| 핵심 로직·상태머신·알고리즘 | `docs/architecture.md` §3 |
| DB 스키마 변경 | `docs/db-schema.md` + `docs/architecture.md` §4 |
| SDK 연동 방식 | `docs/sdk.md` + `docs/architecture.md` §5 |
| store 인터페이스 | `docs/architecture.md` §6 |
| 화면·컴포넌트 스펙 | `docs/architecture.md` §7 |
...
2. 변경 영향 분석:
   - 기존 invariant 깨지는 코드 있는가? → 영향 받는 impl task list
...
**1. docs/prd.md 불일치 발견 시** → architect 직접 수정 X. 결론 `PRODUCT_PLANNER_ESCALATION_NEEDED` + 본문에 (현재 docs/prd.md 내용 / 실제 구현 / 권고) 명시.
