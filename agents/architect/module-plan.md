...
**평소대로 한다**: impl 파일 = 가장 관련 있는 기존 에픽의 impl 폴더에 작성. CLAUDE.md = 기존 에픽 행에 impl 번호 + 이슈 번호 추가. docs/architecture.md 업데이트 (해당 시).
...
- **`## 수용 기준` 섹션 없는 impl 작성 금지** — engineer 가 진입 시 `SPEC_GAP_FOUND` emit → architect SPEC_GAP cycle 로 보강
- 모든 행에 검증 방법 태그 필수
- REQ-NNN 형식 (001부터, 모듈 내 독립 순번)
- **통과 조건 = 실행 가능 커맨드 1+ 줄 의무** (jha0313 패턴 흡수, dcness 하네스 책임). 자연어 ("~가 동작해야 한다", "~가 표시된다") 만 박는 것 금지 — validator code-validation 이 자동 실행해서 검증할 수 있어야 함.

| 태그 | 의미 | 사용 조건 | 통과 조건 형식 (실행 가능 커맨드) |
|---|---|---|---|
| `(TEST)` | vitest 자동 테스트 | 기본값 — 로직·상태·훅 검증 | `pnpm test path/to/file.test.ts` 또는 `pytest tests/test_foo.py::test_bar` 같은 실제 실행 명령 + 기대 결과 (예: `... 5 passed`) |
| `(BROWSER:DOM)` | Playwright DOM 쿼리 | UI 렌더링·DOM 상태 직접 확인 시 | `pnpm playwright test e2e/foo.spec.ts` 같은 실행 명령 + DOM 어설션 기대값 |
| `(MANUAL)` | curl/bash 수동 절차 | 자동화 불가 시만 (이유 명시 필수) | `curl -X POST http://...` / `bash scripts/manual_check.sh` 같은 직접 실행 명령 + 기대 출력 |

**예시 — 실행 가능 커맨드 통과 조건**:

| REQ | 내용 | 검증 방법 | 통과 조건 |
|---|---|---|---|
| REQ-001 | login() 이 잘못된 비밀번호 시 AuthError throw | (TEST) | `pnpm test src/auth/login.test.ts` → `should throw AuthError on invalid password` 통과 |
| REQ-002 | 로그인 폼 제출 시 에러 메시지 노출 | (BROWSER:DOM) | `pnpm playwright test e2e/login.spec.ts -g "shows error"` 통과 |
| REQ-003 | DB 마이그레이션 후 users 테이블 password_hash 컬럼 존재 | (MANUAL) | `psql ... -c "\d users" | grep password_hash` → 1+ 매치 |

**금지** (자연어만 박는 안티패턴):
- ❌ "로그인이 정상 동작한다"
- ❌ "에러 시 사용자에게 메시지를 보여준다"
- ❌ "성능이 저하되지 않는다"
- ✅ 위 형식의 실행 가능 커맨드 + 기대 결과
...
- **AC 모든 행 통과 조건 = 실행 가능 커맨드** — 자연어만 박힌 행 0 건. 미충족 시 SPEC_GAP cycle 진입
- **도메인 모델 정합 검증**:
...
module plan 에 외부 도구 (jest / tsconfig / eslint / vite / metro / babel / package.json scripts 등) config 키 등장 시 의심하면 [`docs/plugin/known-hallucinations.md`](../../docs/plugin/known-hallucinations.md) 카탈로그 확인 또는 공식 docs WebFetch 권고. 자율 판단 — 강제 X.
