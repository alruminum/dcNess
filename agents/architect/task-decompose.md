...
## Story 이슈 생성 의무 (READY_FOR_IMPL 직전)

impl 파일 작성 완료 후, READY_FOR_IMPL 출력 전에 각 story 에 대해 GitHub 이슈 생성:

1. `docs/epic-index.md` Read → 부모 epic 이슈 번호 조회 (없으면 EPIC00).
2. `mcp__github__create_issue` — 제목 `[story] <story명>`, 레이블 `V0N` + `EPIC0N` + `Story0N`, body = story 수용 기준 요약.
3. impl 파일 H1 에 이슈 번호 반영: `# impl/NN — [Story Xa / #이슈번호] …` (이미 있는 규칙과 동일).
4. `stories.md` 의 해당 Story 라인에 `관련 이슈: #N` 추가.
5. 이슈 생성 실패 시 prose 에 `GITHUB_ISSUE_MISSING: Story X` 명시 후 계속 진행.

**레이블 형식**: `V01` (버전, 2자리 zero-pad) + `EPIC01` (에픽, 2자리) + `Story01` (스토리 전역 고유 번호, 2자리). 동적 레이블 — 존재하지 않으면 GitHub 이 자동 생성.
