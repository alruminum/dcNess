...
UI 컴포넌트 포함 epic 분해 시, 다음 둘 중 하나 = **듀얼 모드**:
- `docs/design.md` 미존재
- `docs/design.md` 의 `components` 섹션 (frontmatter 또는 본문 §Components) 에 본 epic 의 UI 컴포넌트 미정의
...
`docs/design.md` 의 `components` 섹션에 본 epic UI 컴포넌트가 정의된 경우 (= 디자인 후 구현 모드) 는 본 가드레일 스킵.
