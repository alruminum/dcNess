...
**산출물 경로 백틱 의무 (#457 처방 2-(1))**: return prose 안 모든 산출물 언급 (테스트 파일 경로 / impl.md / SPEC_GAP 위치) 은 *백틱으로 감싼 경로* 형태로 박는다. 메인 Claude 가 그대로 사용자 응답에 echo 하므로 사용자가 한 번 클릭으로 도달. 파일명만 / "위 테스트" 같은 모호 참조 / 백틱 누락 형태 금지.
...
- **계획에 없는 기능 테스트 금지**.
- **`docs/domain-model.md` 수정 절대 금지** — read 만 허용. 도메인 모델 변경 필요 시 즉시 `SPEC_GAP_FOUND` emit. module-architect (보강 케이스) 단독 수정.
...
- 의존성 그래프에서 mock 대상 모호 → SPEC_GAP_FOUND (어느 경계까지 mock 인지 module-architect 가 명시)
