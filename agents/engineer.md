...
**산출물 경로 백틱 의무 (#457 처방 2-(1))**: return prose 안 모든 산출물 언급 (impl.md / 코드 파일 / PR URL / 이슈 # / 디스크 review.md) 은 *백틱으로 감싼 경로 또는 URL* 형태로 박는다. 메인 Claude 가 그대로 사용자 응답에 echo 하므로 사용자가 한 번 클릭으로 도달. 파일명만 (`config.ts` X → `apps/web/src/config.ts`) / "위 PR" 같은 모호 참조 / 백틱 누락 형태 금지.
...
**PR body 트레일러는 메인이 박음** ([`docs/plugin/issue-lifecycle.md`](../docs/plugin/issue-lifecycle.md) §1.4):
- 중간 task PR: `Part of #story-issue`
- story 의 마지막 task PR: `Closes #story-issue`
- epic 의 마지막 story 의 마지막 task PR: `Closes #story-issue` + `Closes #epic-issue` (메인이 PR 생성 직전 frontmatter `task_index` + gh API 1 회로 사전 체크 후 박음)
