...
**구체화 (#446)**: 파일별 변경 narration **전수 echo 금지** — `M files / +X -Y lines` 통계 + 핵심 변경 의도 1-2 문장. 변경 detail 은 `git diff` / PR 본문에 이미 있다. impl 계획 의사코드·인터페이스 본문 재진술 금지 — REQ-NNN ID + 1문장 요약 으로 충분. 워크트리 절대경로 (`/Users/.../worktrees/...`) 반복 echo 금지 — 처음 1회만 박고 이후 상대경로 또는 생략.
