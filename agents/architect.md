...
System Design / Module Plan 완료 후 다음 변경 시 `docs/architecture.md` 업데이트:

| 변경 유형 | 업데이트 대상 |
|---|---|
| 기술 스택 추가/변경 | docs/architecture.md 기술 스택 섹션 |
| 프로젝트 구조 변경 | docs/architecture.md 프로젝트 구조 섹션 |
| 핵심 로직·상태머신·알고리즘 변경 | docs/architecture.md 핵심 로직 섹션 |
| DB 스키마 변경 | docs/architecture.md DB 섹션 + docs/db-schema.md |
| SDK/외부 API 연동 변경 | docs/architecture.md SDK 섹션 + docs/sdk.md |
| 전역 상태 인터페이스 변경 | docs/architecture.md 전역 상태 섹션 |
| 화면 구성/컴포넌트 스펙 변경 | docs/architecture.md 화면 컴포넌트 섹션 |
| 환경변수 추가/변경 | docs/architecture.md 환경변수 섹션 |

업데이트 방법: `docs/architecture.md` 해당 섹션 수정 + 변경 이력 한 줄 추가. 소규모 수정 (오타·문구) 은 변경 이력 생략 가능. 인터페이스·로직·스키마 변경은 항상 이력 추가.

## 프로젝트 특화 지침

작업 시작 시 프로젝트 루트 `CLAUDE.md` 의 아키텍처 섹션 매핑 + 추가 지침 Read 로 읽어 적용. 별도 `agent-config/` 별 layer 없음.
