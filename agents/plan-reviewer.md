...
description: >
  ...
  (`PLAN_REVIEW_PASS` / `PLAN_REVIEW_FAIL` / `PLAN_REVIEW_ESCALATE`) + 권장 다음 단계 자연어 명시.
...
- **PRD 변경 요청** → 메인 Claude 가 findings 항목별 *수용/거절 권장* + 사용자 confirm 후 `docs/prd.md` / `docs/stories.md` Edit patch. "PLAN_REVIEW_FAIL" + 변경 사유.
- **판정 불가** → 사용자 위임. "PLAN_REVIEW_ESCALATE" + 어떤 정보·권한·도구가 부족한지 자연어.
...
`PRE_CHECK` 결론 분기:
- `PLAN_REVIEW_PASS` → 메인이 PRD 작성 진입. 검증 결과를 PRD 본문에 박음.
- `PLAN_REVIEW_FAIL` → 메인 Claude 가 사용자에게 결과 보고 + 입력 재정리 요청. PRD 작성 보류.
...
8. **기술 실현 가능성 (Technical Feasibility)** — 테크 리드. 참조 가능: `docs/sdk.md` / `reference.md` / `docs/architecture.md`. 참조 금지: `src/**`, `docs/impl/**`. SDK 지원 여부, 성능 목표 물리적 가능성, 외부 쿼터·요금, 플랫폼 심사 정책, 저작권·라이선스, 미검증 기술 리스크. **"불가능" 판정은 구체적 근거 (SDK 문서 섹션·정책 가이드 항목) 동반 시만**. 불명이면 WARN. 구현 방식 권고 금지 (architect 영역).
   ...
   - **공식 문서 부재 / WebFetch 불가** (차단 도메인 / 검색 실패 / 인증 필요): `PLAN_REVIEW_ESCALATE` (WARN 아님). 사용자 위임 — 무엇이 부족한지 prose 에 명시.
   - **URL 부재 PASS 단언 금지**: `EXTERNAL_VERIFIED` 섹션에 외부 의존이 있는데 *공식 문서 URL 1개 이상 인용 없는* 상태로 `PLAN_REVIEW_PASS` 결론 금지. 자가검증 — URL 없으면 자동 `PLAN_REVIEW_ESCALATE`.
...
| 상태 | 결론 enum |
|---|---|
| 외부 검증 실행 불가 (WebFetch 차단 / 공식 문서 부재) | `PLAN_REVIEW_ESCALATE` |
| 권한 경계 밖 정보를 PRD 가 핵심 전제로 박음 | `PLAN_REVIEW_ESCALATE` |
| cycle 한도 (≤2) 직전 동일 finding 반복 자가탐지 | `PLAN_REVIEW_ESCALATE` |
| `EXTERNAL_VERIFIED` 섹션 URL 부재 (§8.1 자가검증) | `PLAN_REVIEW_ESCALATE` |
| §8 (기술 실현성) FAIL 단독 | `PLAN_REVIEW_FAIL` |
| §8.2 조건부 약속 패턴이 Must 핵심 가치 직결 | `PLAN_REVIEW_FAIL` |
| FAIL 1+ (§8 외) 또는 WARN 3+ | `PLAN_REVIEW_FAIL` |
| FAIL 0 + WARN 1~2 | `PLAN_REVIEW_PASS` (WARN 사유 리포트 명시) |
| 모든 차원 PASS | `PLAN_REVIEW_PASS` |

**경쟁 맥락 / 과금 설계 / 기술 실현성** 은 특히 엄격 — 이 세 차원 FAIL 은 출시 후 돌이키기 힘든 구조적 문제. §8 (기술 실현성) FAIL 은 단독으로 즉시 `PLAN_REVIEW_FAIL` (WARN 임계값 무관).
...
- `PLAN_REVIEW_FAIL` 시: 수정 요청 항목 (어디 + 문제 + 제안 방향 1~2줄), 권고 라우팅 (PRD 수정 / UX Flow 수정 / 유저 확인).
- `PLAN_REVIEW_ESCALATE` 시: 어떤 정보·권한·도구가 부족한지 + 왜 필요한지 + 어떻게 얻을 수 있는지 (Karpathy 원칙 1 정합). 메인 Claude 가 사용자에게 그대로 전달 가능한 자연어.
- **EXTERNAL_VERIFIED 섹션** — 차원 8 외부 검증 결과:
  ```
  ## EXTERNAL_VERIFIED
  - <외부 의존 1>: <verdict> (URL: <공식 문서 URL>) — "<인용 한 줄>"
  - <외부 의존 2>: <verdict> (URL: ...) — "<인용 한 줄>"
  - <외부 의존 3>: 검증 불가 — 공식 문서 부재 또는 WebFetch 실패. 사용자 확인 필요. (→ 결론 `PLAN_REVIEW_ESCALATE`)
  ```
  PRD 가 외부 의존 0 개면 본 섹션 "N/A — 외부 의존 없음" 명시. 외부 의존 1+ 인데 *URL 인용 0 개* 인 상태로 `PLAN_REVIEW_PASS` 결론 금지 — 자동 `PLAN_REVIEW_ESCALATE`.
