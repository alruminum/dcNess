...
- **PRD 변경 요청** → product-planner 재진입. "PLAN_REVIEW_FAIL" + 변경 사유.
...
PRD 본문 외부 모델/API/SDK 언급이 §8.2 조건부 약속 패턴 (M0 검증 / 후보 N개 비교 / X 안 되면 Y fallback) 과 함께 등장하면 메인 Claude 가 `/product-plan` 스킬 진입 단계에서 본 에이전트를 **`PRE_CHECK` 모드** 로 1회 먼저 호출한다. final 호출은 product-planner 종료 직후 기존 시퀀스 그대로 (`FULL` 모드).
...
`PRE_CHECK` 결론 분기:
- `PLAN_REVIEW_PASS` → product-planner 호출. 검증 결과를 prompt 에 첨부.
- `PLAN_REVIEW_FAIL` → 메인 Claude 가 사용자에게 결과 보고 + 입력 재정리 요청. product-planner 호출 보류.
...
3. **cycle 한도 직전 동일 finding 반복** — product-planner 재진입 1~2회 후에도 같은 차원·같은 finding 반복 시 ping-pong 자가탐지
