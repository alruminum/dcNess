...
- 실 ANTHROPIC_API_KEY 보유 (haiku interpreter 실 호출용)
...
export DCNESS_LLM_TELEMETRY=1   # 기본값, 명시
...
result = interpret_with_fallback(
    ...
    llm_interpreter=make_haiku_interpreter(),
)
...
**기대**:
- `Result: PASS` (휴리스틱 hit, LLM 미호출)
- `.metrics/heuristic-calls.jsonl` 에 `outcome: "heuristic_hit"` 1줄
- analyzer 가 `cycle 당 메타 LLM 비용 < $0.10 ✅ PASS` 판정

**추가**: 의도적 ambiguous 케이스로 LLM fallback 유도:

```python
prose = "검증 진행 중. 결론 미확정."  # 휴리스틱 0 hit → LLM
result = interpret_with_fallback(prose, ["PASS", "FAIL"], llm_interpreter=make_haiku_interpreter())
```

기대: LLM 호출 발생, `.metrics/meta-llm-calls.jsonl` 에 cost ~$0.0001 entry, analyzer 가 `LLM fallback rate` 출력.
...
| 메타 LLM 비용 cycle 당 < $0.10 | `analyze_metrics.mjs` `cost_projection.per_cycle_65_calls_usd` |
...
- 메타 LLM cycle 당 — < $0.10 (본 가이드 §6 fitness)
- catastrophic 가드 — 무손실
