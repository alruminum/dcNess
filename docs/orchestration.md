...
> **Status**: ACTIVE
> **Origin**: `DCN-CHG-20260429-25` (Phase 3 사후 보강) + `DCN-CHG-20260505-03` (loop-catalog 흡수, 8 → 7 SSOT)
...
- enum 해석 = `harness/interpret_strategy.py` heuristic-only (DCN-CHG-20260430-04, LLM fallback 폐기 — ambiguous 시 메인이 cascade)
...
5. **architect TASK_DECOMPOSE 직전 validator DESIGN_VALIDATION (DESIGN_REVIEW_PASS) 없이 진입 금지** (DCN-CHG-20260430-05)
...
DESIGN_VALIDATION cycle 한도 = 2 (DCN-CHG-20260430-05). 초과 시 사용자 위임. catastrophic §2.3.5 — TASK_DECOMPOSE 직전 DESIGN_REVIEW_PASS 필수.
...
## 6. 코드 Driver — 메인-주도 컨베이어 + PreToolUse 훅 (`DCN-CHG-20260429-29`)
...
상세 디자인 + 폐기된 옵션 카탈로그 (3 옵션 — RWHarness fork / 정적 dict / Orchestration Agent) = [`conveyor-design.md`](conveyor-design.md).
...
- [`conveyor-design.md`](conveyor-design.md) — 코드 driver 디자인 + 폐기된 옵션 카탈로그
...
- [`process/plugin-dryrun-guide.md`](process/plugin-dryrun-guide.md) — plugin 배포 dry-run
- [`migration-decisions.md`](migration-decisions.md) §2.1 — RWHarness 모듈 분류 (impl_loop.py DISCARD)
