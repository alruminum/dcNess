...
> **🔴 대 원칙** ([`plugin/prose-only-principle.md`](plugin/prose-only-principle.md) §1 직접 인용):
...
본 SSOT 는 위 2 개 강제 영역만 정의. 형식 강제 (마커 / status JSON / Flag) 는 [`plugin/prose-only-principle.md`](plugin/prose-only-principle.md) 에 의해 폐기 — 본 문서 안에서도 그 어휘는 사용하지 않는다.
...
**sub_cycles**: 없음. AMBIGUOUS 시 [`plugin/skill-guidelines.md`](plugin/skill-guidelines.md) §6 cascade.
...
- [`plugin/prose-only-principle.md`](plugin/prose-only-principle.md) — 본 SSOT 의 강제 영역 정의 현행 SSOT (대 원칙 + Anti-Pattern 5원칙)
...
- [`plugin/skill-guidelines.md`](plugin/skill-guidelines.md) — cross-cutting 룰 (echo / yolo / self-verify)
