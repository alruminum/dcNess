...
- **Deep**: high-risk trigger 가 있거나 새 epic/product feature 처럼 사전 설계 합의가 필요하다. `/spec` / `/tech-review` 필요 시 / `/design` / `/impl` / `/acceptance` 흐름을 사용한다.
...
| **Deep** | high-risk trigger 1개 이상 또는 새 epic/product feature | `/spec` → `/tech-review` 필요 시 → `/design` → `/impl` → `/acceptance` (deep task 파일이 있으면 `/impl-loop` advanced 위임) | 되돌리기 비싼 결정 → 설계·검증 consensus 필요 |
