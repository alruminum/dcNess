...
기본 public surface 는 `/spec -> /design -> /impl -> /acceptance` 다. 이 네 단계는 제품 생명주기 이름이고, Lite / Standard 는 `/impl` 안의 내부 구현 lane 이다. high-risk 작업은 `/impl` 내부 lane 이 아니라 `/impl` 진입 *전* 설계 선행(`/spec`·`/design`)으로 라우팅된다. lane 별 command 를 public surface 로 늘리지 않는다. 기본 표면 계약은 [`positioning.md`](positioning.md) 가 진본이다.
...
- **Standard** (`/impl` 내부): 설계 문서(경로)가 들어온 lane 이다. impl 은 설계를 만들지 않고 받은 설계도로 구현만 한다 — 경량 설계 산출은 impl 밖 [`compact-design`](../../skills/compact-design/SKILL.md)(`module-architect` 1-pass) 또는 full `/design` 이 담당하고, 그 산출물 경로가 진입 prerequisite 다. 엔진(풀4/경량 build-worker)은 lane 과 직교다.
- **high-risk → 설계 선행** (`/impl` 내부 lane 아님): high-risk trigger 가 있거나 새 epic/product feature 처럼 사전 설계 합의가 필요하다. impl 진입 *전* `/spec` 내부 tech-review preflight 필요 시 / `/design` / `/impl` / `/acceptance` 흐름으로 설계를 선행하고, 산출된 설계도를 들고 `/impl` Standard 로 진입한다.
...
| **high-risk → 설계 선행** (impl 내부 lane 아님) | high-risk trigger 1개 이상 또는 새 epic/product feature | impl 진입 *전* `/spec` 내부 tech-review preflight 필요 시 → `/design` → `/impl` → `/acceptance` (deep task 파일이 있으면 `/impl-loop` advanced 위임) | 되돌리기 비싼 결정 → 설계·검증 consensus 필요 |
