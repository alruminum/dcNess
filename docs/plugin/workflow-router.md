...
- **issue-report (qa 분류)** = *아직 분류 안 된 새 버그/이슈 신고* 를 분류한다 (KNOWN_ISSUE / DESIGN_ISSUE / SCOPE_ESCALATE 등). 짧게는 **미분류 버그 접수**다.
...
acceptance gap issue 는 이미 제품 검수에서 근거가 생긴 후속이므로 새 미분류 버그 접수로 되돌리지 않는다. 사용자 승인 후 issue 를 만들었다면 concrete signal 로 보고 `/impl`, `/design`, `/spec`, `/ux` 등 해당 후속 라우팅으로 진입한다.
