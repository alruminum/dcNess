...
**차단 동작**: handler 가 정책 위반 시 `RC=1` 반환 → wrapper 가 `exit 2` 로 번역 → CC 가 Agent 호출 거부 + stderr 메시지 노출. 메인이 회복 또는 사용자 위임. (`exit 1` 은 CC 가 non-blocking 으로 취급해 차단 안 됨 — wrapper 번역이 필수. §2.1.4 진입 gate 는 메인 prose 가 자율 보존)
