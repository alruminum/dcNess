...
> **§2.1.4 단방향 catastrophic** 만 코드 강제가 아닌 *자연어 룰* — `commands/tech-review.md` / `skills/architect-loop/SKILL.md` 가 기능 참조. architect-loop 도중 tech-review 미검증 새 외부 의존 발견 시 처리 = `NEW_DEP_ESCALATE` 3안 (채택+수동검증 / 대안 우회 / 전체 회귀 — 어느 경우든 tech-reviewer 재호출 0). 라우팅 = [`routing.md`](routing.md) §3 escalate.
