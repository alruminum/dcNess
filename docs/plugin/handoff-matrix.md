...
다음 결론 enum 수신 시 **메인 Claude / driver 가 즉시 사용자 보고 후 대기** ([`prose-only-principle.md`](prose-only-principle.md) §2 원칙 2 정합 — 자동 복구 금지):
...
> RWHarness `harness-architecture.md` §3 의 dcNess 변환. dcness 의 두 번째 강제 영역 = "접근 영역" ([`prose-only-principle.md`](prose-only-principle.md) §1 정합).
...
> RWHarness 의 `HARNESS_INFRA_PATTERNS` 안 `r'orchestration-rules\.md'` 잔재는 dcness 가 정정 — 파일명은 `docs/plugin/orchestration.md` + `docs/plugin/handoff-matrix.md` (split 후 양쪽). loop-procedure / dcness-rules / self-guidelines / hooks·session_state·agent_boundary 도 dcness 가 추가 보호.
...
- [`prose-only-principle.md`](prose-only-principle.md) — Prose-Only 원칙 현행 SSOT (대 원칙 + Anti-Pattern 5원칙)
