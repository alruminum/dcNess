...
**대상 + 읽을 진본:** {{이번 호출이 다룰 단위 + 그 진본 경로 (agent 가 자체 read).
  산출 경로·write 경계가 진본(task 파일 Scope · 이슈)에 있으면 재기입하지 않는다.
  예) Lite=이슈 #NN · std/deep=task 파일 경로 · system-architect=root·epic SSOT
...
**이 호출 특유:** {{진본에 아직 없는 것만 — 미기록 결정·신호(그릴미 합의 · Cross-Story
  Lessons · wave-plan 신호 · 재호출 finding relay). 없으면 생략.
  ❌ 방법(어떻게 만들/고치라 — 정규식·구현 단계·알고리즘) 처방 금지: 무엇/어느 단위만, 방법은 agent.}}
```
