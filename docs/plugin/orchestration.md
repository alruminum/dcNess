...
- CLAUDE.md §0 정합 — "architect / code-validator / engineer 위임 강제 없음"
...
5. **architect MODULE_PLAN × N (feature-build-loop §4.2 마지막 단계) 진입 직전 architecture-validator PASS 없이 진입 금지**
...
flowchart LR
    ...
    ux -->|UX_FLOW_READY| sd[architect SYSTEM_DESIGN<br/>+ 5 항목 self-check<br/>+ impl 목차 표]
    ux -->|UX_FLOW_ESCALATE| esc((escalate))
    sd -->|SYSTEM_DESIGN_READY| dv[architecture-validator<br/>Placeholder Leak + Spike Gate]
    dv -->|PASS| mp[architect MODULE_PLAN × N<br/>impl 목차 행마다 1 호출]
    dv -->|FAIL| sd
    dv -->|ESCALATE| esc
    mp -->|READY_FOR_IMPL × N| impl[구현 루프 §2.1]
```

UX 검증은 ux-architect self-check (5 카테고리, FAIL 시 재고려 2 cycle) 로 흡수. architecture-validator cycle 한도 = 2. 초과 시 사용자 위임. catastrophic §2.3.5 — MODULE_PLAN × N 진입 직전 architecture-validator PASS 필수.
...
|------|-------------|--------------------|---------|------------|----------------|
| `feature-build-loop` (§3.1, §4.2) | `product-plan` | product-planner / plan-reviewer / ux-architect:UX_FLOW (self-check) / architect:SYSTEM_DESIGN (self-check) / architecture-validator / architect:MODULE_PLAN × N | `PRODUCT_PLAN_READY` → `PLAN_REVIEW_PASS` → `UX_FLOW_READY` → `SYSTEM_DESIGN_READY` → `PASS` → `READY_FOR_IMPL × N` | advance 동일 | 5 + N (N = SD impl 목차 행 수) |
| `impl-task-loop` (§2.1, §4.3) | `impl` | (default) test-engineer / engineer:IMPL / code-validator / pr-reviewer · (fallback: impl 부재 시 architect:MODULE_PLAN 선두 추가) | `TESTS_WRITTEN` → `IMPL_DONE` → `PASS` → `LGTM` | advance 동일 | 4 (default) / 5 (fallback) |
| `impl-ui-design-loop` (§2.2, §4.4) | `impl` (UI 감지) | (default) designer / design-critic / test-engineer / engineer:IMPL / code-validator / pr-reviewer · (fallback: impl 부재 시 architect:MODULE_PLAN 선두 추가) | `DESIGN_READY_FOR_REVIEW` → `VARIANTS_APPROVED` → `TESTS_WRITTEN` → `IMPL_DONE` → `PASS` → `LGTM` | advance 동일 | 6 (default) / 7 (fallback) |
| `quick-bugfix-loop` (§3.5, §4.5) | `quick` | qa / architect:LIGHT_PLAN / engineer:IMPL / code-validator / pr-reviewer | `FUNCTIONAL_BUG`/`CLEANUP` → `LIGHT_PLAN_READY` → `IMPL_DONE` → `PASS` → `LGTM` | advance 동일 | 5 |
...
| 5 | architect:SYSTEM_DESIGN (5 항목 self-check 의무) | `SYSTEM_DESIGN_READY,TECH_CONSTRAINT_CONFLICT` (산출물에 `## impl 목차` 표 포함 — Story → impl 매핑 + N 행) |
| 5.5 | architecture-validator | `PASS,FAIL,ESCALATE` |
| 6.1 ~ 6.N | architect:MODULE_PLAN (occurrence 1..N — SD impl 목차 행마다 1번) | `READY_FOR_IMPL,SPEC_GAP_FOUND,TECH_CONSTRAINT_CONFLICT` |

**MODULE_PLAN × N 단계 (Step 6)**:
...
**근거**: feature-build-loop §4.2 의 Step 6 (architect MODULE_PLAN × N) 이 정식 위치 impl 파일 본문 detail 까지 채움. 즉 정식 경로 + 파일 존재 = 본문 detail 보장 + architecture-validator PASS 통과물. MODULE_PLAN 재호출 redundant. (이전 `MODULE_PLAN_READY` 마커 grep 룰 — DCN-30-13 — 폐기. 위치 자체가 도장.)
...
- code-validator `ESCALATE` → 본문 사유 prose 보고 메인 분기: spec 부재면 architect SPEC_GAP, 재시도 한도 초과면 사용자 위임
...
`/impl-loop` = `impl-task-loop` × N. outer task `impl-<i>: <task>` + inner sub-task `b<i>.<agent>` (DCN-CHG-30-12). default = 4 sub-task (test-engineer / engineer / code-validator / pr-reviewer), fallback = 5 (architect:MODULE_PLAN 선두 추가). 각 task clean → 자동 7a + 다음 task. caveat → 멈춤 + 사용자 위임 (Step 2.5 — `commands/impl-loop.md` 참조).
