...
| ux-architect | UX_FLOW_READY → system-architect / UX_REFINE_READY → designer / UX_FLOW_ESCALATE → 사용자. **UI-less epic 이면 메인이 호출 안 함** (§4.2 UI-less 분기) |
...
escalate 결론 enum (`IMPLEMENTATION_ESCALATE` / `UX_FLOW_ESCALATE` / `ESCALATE` / `SCOPE_ESCALATE` / `NEW_DEP_ESCALATE`) 수신 시 **메인 / driver 가 즉시 사용자 보고 후 대기** (§0 — 자동 복구 / 우회 / 재시도 금지). 각 enum 의 출처 agent·의미 = §3.1 표 + 해당 agent 본문. escalate 분기 풀스펙(architect-loop) = §4.2 분기.

`NEW_DEP_ESCALATE` (system-architect / module-architect — architect-loop 도중 tech-review 미검증 새 외부 의존 발견) 만 예외적으로 "단순 대기"가 아니라 메인이 사용자에게 **3안 메뉴** 제시 — (1) 채택+수동검증 → architect 재진입 / (2) 대안 기술 우회 → architect 재진입 / (3) 전체 원점 회귀 (`/architect-loop` 중단 + `/product-plan` 재진입 + 새 tech-review). (1)/(2) cycle ≤ 2. **어느 옵션이든 tech-reviewer 재호출 없음 (§2.1.4 단방향 catastrophic 보존)**. 흐름 = §4.2 분기 + [`commands/architect-loop.md`](../../commands/architect-loop.md) `## 분기 / cycle (요약)`.
...
> 🔴 **라우팅 진본 (1-way)** — `agent 결론 → 다음 호출` 매핑 진본 = §3.1 routing 한눈표. 본 §4 는 loop 조립(시퀀스) view — 라우팅 매핑 미중복 (step 순서 / allowed_enums / commit 지점 등 loop 고유 정보만 보유). 라우팅 갱신은 §3.1 한 곳만.

### 4.1 한눈 인덱스

> 🔴 **진본 이전**: loop 한눈 인덱스 (entry_point / task_list / advance / clean_enum / expected_steps) = [`loop-procedure.md`](loop-procedure.md) §7.0.
...
- system-architect / module-architect `NEW_DEP_ESCALATE` (tech-review 미검증 새 외부 의존) → loop 자동 중단 X. 메인이 사용자에게 3안 제시 — (1) 채택+수동검증 → architect 재진입 (architecture.md/adr.md 에 "사용자 승인, tech-review 미경유" 흔적), (2) 대안 기술 우회 → architect 재진입, (3) 전체 원점 회귀 (`/architect-loop` 중단 + `/product-plan` 재진입 + 새 tech-review). (1)/(2) cycle ≤ 2. **어느 옵션이든 tech-reviewer 재호출 없음 (§2.1.4 단방향 보존)**. 상세 = [`commands/architect-loop.md`](../../commands/architect-loop.md) `## 분기 / cycle (요약)` + §3.3
...
**분기**:
- `IMPL_PARTIAL` → engineer IMPL 재호출 (분할 — 새 context window. **retry 아님 = 상한 없음, attempt 미소비** §3.2 / DCN-30-38). prose 의 *남은 작업* 컨텍스트로 진입.
...
- `engineer IMPL` 분할 (IMPL_PARTIAL 시, 상한 없음 — retry 아님, DCN-30-38) — engineer IMPL 동일. prose 의 `## 남은 작업` 컨텍스트로 진입.
