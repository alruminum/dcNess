...
전역 `docs/architecture.md` 의 집계 섹션은 사용자 repo 에 복사하지 않는 plugin script `$PLUGIN_ROOT/scripts/aggregate_architecture_map.mjs` 가 생성/검증한다. 활성 프로젝트에서는 이 스크립트를 현재 프로젝트 루트에서 실행한다.
...
- 포함: `git-naming-validation.yml`, `pr-body-validation.yml`, `doc-path-integrity.yml`, `github-project-lifecycle.yml`
