...
핸들러:
    ...
    handle_pretooluse_agent(stdin_data, cc_pid) -> exit_code
        PreToolUse, tool=Agent. HARNESS_ONLY_AGENTS + §2.3 룰 검사.
        exit 0 = allow, exit 1 = block (stderr 메시지 + CC 가 호출 거부).

§2.3.4 (PRD 변경 후 plan-reviewer PASS) / §2.3.5 (module-architect 진입
직전 architecture-validator PASS) 는 옛 단일 `architect` agent 시절 mode
체크 잔재였음. 현재 system-architect / module-architect 분리 + architect-loop
신설 (2026-05-12) 후 prerequisite 검증은 *메인 영역* (skill 안에서 보장).
코드 강제는 §2.3.1 (pr-reviewer 직전 code-validator PASS) / §2.3.3 (engineer
직전 module-architect READY) / §2.3.6~§2.3.8 (impl-task-loop 3-commit) 만.
...
def handle_pretooluse_agent(
    ...
    *,
    ...
    # §2.3.4 / §2.3.5 — 옛 단일 `architect` agent + mode 시절 catastrophic gate.
    # system-architect / module-architect 분리 + architect-loop 신설 (2026-05-12)
    # 후 prerequisite 검증은 메인 영역 (architect-loop skill 안에서 보장 — Pre-flight
    # gate / 전제 조건). 코드 강제 폐기.
