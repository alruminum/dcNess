...
(skill 안 Pre-flight) 또는 다른 흐름 (`/architect-loop` 의 impl 미리 머지 등)
으로 이전 — 코드 강제 폐기. 본 hook 코드 강제는 3 게이트:
pr-reviewer 게이트 (직전 code-validator PASS) / engineer 게이트 (직전
module-architect PASS) / module-architect 게이트 (architect-loop 안 첫 호출
...
def handle_pretooluse_agent(
    ...
    *,
    ...
    # pr-reviewer 게이트 — pr-reviewer 직전 code-validator PASS 필수
    if subagent == "pr-reviewer":
        if _has_engineer_write(rd) and not _has_pass(rd, "code-validator"):
            print(
                "[catastrophic: pr-reviewer 게이트] pr-reviewer 호출은 code-validator "
                "PASS 후만",
                file=sys.stderr,
            )
            return 1

    # module-architect 게이트 — architect-loop 안 module-architect × K *첫 호출* 직전
