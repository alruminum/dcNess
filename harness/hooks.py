...
강제 폐기. 본 hook 코드 강제는 §2.3.1 (pr-reviewer 직전 code-validator PASS)
/ §2.3.3 (engineer 직전 module-architect PASS) / §2.3.5 (architect-loop 안
...
def handle_pretooluse_agent(
    ...
    *,
    ...
    # §2.3.1 — pr-reviewer 직전 code-validator PASS 필수
    ...
    # §2.3.5 — architect-loop 안 module-architect × K *첫 호출* 직전
    ...
    if (
        ...
        if not _has_pass(rd, "architecture-validator"):
            print(
                "[catastrophic §2.3.5] module-architect × K 첫 호출은 "
    ...
    # §2.3.2 (LGTM 없이 merge — 자연어 폐기) / §2.3.4 / §2.3.6~§2.3.8 (3-commit) —
...
def _is_architect_loop(
    ...
    *,
    ...
    §2.3.5 catastrophic gate 발동 조건 — architect-loop 안 module-architect
