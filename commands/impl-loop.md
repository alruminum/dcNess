...
2. **task 한 개씩** — 아래 a~e 를 task N 이 완전히 끝난 (PR 머지 + 이슈 close) 뒤에만 task N+1 진입. 한 번에 한 task.
   ...
   3. **git/PR 생성 (메인)** — worker prose 의 commit message + PR 본문 초안을 임시 파일로 박고 `scripts/pr-create.sh` 통합 호출:
      ...
      ```
      `branch_prefix` = `orchestration.md §4.3 branch_prefix decision rule` 정합. base 분기 = `git-naming-spec.md §6` 정합 (통합 브랜치 모드 매치 시 그 값). 분리 명령 (`git checkout -b` / `git add` / `git commit` / `git push` / `gh pr create` 각각 호출) 은 *비권장* — 메인 turn 누적 회수 영역.
