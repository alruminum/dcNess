...
2. **task 한 개씩** — 아래 a~d 를 task N 이 완전히 끝난 (PR 머지 + 이슈 close) 뒤에만 task N+1 진입. 한 번에 한 task.
   ...
   4. **end-run + review 출력 재정의 (5줄 요약)** — [`commands/impl.md`](impl.md) §종료 조건 따름. 단 메인 컨텍스트에 박는 echo 는 본 skill §"review 출력 재정의 (#446)" 의 5줄 요약본으로 *재정의* — 원본 review.md 는 `<run_dir>/review.md` 디스크 그대로 (이미 저장).
3. **enum 별 분기**:
   - `clean` → 다음 task 진입
