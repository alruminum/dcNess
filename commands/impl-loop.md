---
name: impl-loop
description: impl task list (architect-loop 의 module-architect × K 산출물) 를 메인 Claude 한 세션 안에서 오케스트레이션으로 task 한 개씩 순차 처리하는 스킬. 메인이 한 세션 안에서 직렬 진행 — 각 task = 1 PR + 1 이슈 close. 사용자가 "전부 구현", "/impl-loop", "task 다 돌려", "epic 전체 구현", "/architect-loop 후 자동", "끝까지 구현" 등을 말할 때 반드시 이 스킬을 사용한다. 각 task 가 분리 PR = `/run-review` 도 task 별 분리 분석 자연. clean run 만 자동 진행, 걸리는 게 생기면 (사용자 개입 필수) 즉시 정지. /architect-loop 종료 후 K 개 task 한 번에 처리하고 싶을 때.
---

# Impl Loop Skill — 한 세션 안에서 다중 task 오케스트레이션

## Loop
`impl-task-loop × N` — 다중 task chain (본 skill 본문 = 진본). 메인 Claude 가 한 세션 안에서 task 를 한 개씩 순차 진행. 각 task 의 inner loop = `impl-task-loop` — [`commands/impl.md`](impl.md) 본문 의무를 그대로 따른다. loop 인덱스 = [`loop-procedure.md`](../docs/plugin/loop-procedure.md) §7.0.
...
정상 흐름은 본 skill 본문 + 인용된 docs §번호 만으로 진행. 본문에 있는 catastrophic / Pre-flight gate 룰이 1차. *룰 모호 / 분기 발생* 시에만 `docs/plugin/loop-procedure.md` (절차 + §7.0 인덱스) / `commands/impl.md` / `issue-lifecycle.md` 부분 read (grep + offset/limit). 통째 read 폐기 — 메인 cache_read 기준치 감축.
...
2. **task 한 개씩** — 아래 a~e 를 task N 이 완전히 끝난 (PR 머지 + 이슈 close) 뒤에만 task N+1 진입. 한 번에 한 task.
   ...
   3. **git/PR 생성 (메인)** — worker prose 의 commit message + PR 본문 초안을 임시 파일로 박고 `scripts/pr-create.sh` 통합 호출:
      ...
      ```
      `branch_prefix` = [`commands/impl.md`](impl.md) `## branch_prefix 결정` 정합 (feat/chore/fix). base 분기 = `git-spec.md §6` 정합 (통합 브랜치 모드 매치 시 그 값). 분리 명령 (`git checkout -b` / `git add` / `git commit` / `git push` / `gh pr create` 각각 호출) 은 *비권장* — 메인 turn 누적 회수 영역.
   4. **pr-reviewer step + 머지** — `begin-step pr-reviewer` → `Agent(pr-reviewer, prompt=<PR 번호 + impl 경로 + 구현 파일 목록>)` → `PASS` 시 `bash scripts/pr-finalize.sh <PR>` 호출 (gh pr merge --auto + watch + main sync 자동). `FAIL` 시 engineer POLISH 단발 진입 (cycle ≤ 2). 이후 `end-step pr-reviewer`.
      - **provider resolve**: 호출 직전 `PROVIDER=$("$HELPER" routing resolve pr-reviewer)` 실행. `codex` 이면 `"$PLUGIN_ROOT/scripts/dcness-codex-validator" pr-reviewer --prompt-file "$PROMPT_FILE"` 로 대체한다. wrapper 가 `end-step pr-reviewer --prose-file ...` 까지 수행하므로 Codex route 에서는 별도 end-step 중복 호출 금지.
...
- 진입 모드 분기: 본 skill 본문 + [`commands/impl.md`](impl.md)
- phase prose 자체 Write 규약: [`docs/plugin/loop-procedure.md §3.2.1`](../docs/plugin/loop-procedure.md)
- 결론 가이드: [`docs/plugin/routing.md`](../docs/plugin/routing.md) §1 / ALLOW_MATRIX: [`harness/agent_boundary.py`](../harness/agent_boundary.py)
...
## 참고
- 다중 task chain 스펙: 본 skill 본문 (진본)
- 각 task inner loop: [`commands/impl.md`](impl.md) (`impl-task-loop`)
