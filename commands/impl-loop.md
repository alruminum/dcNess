...
정상 흐름은 본 skill 본문 + 인용된 docs §번호 만으로 진행. 본문에 박힌 catastrophic / Pre-flight gate 룰이 1차. *룰 모호 / 분기 발생* 시에만 `docs/plugin/loop-procedure.md` / `orchestration.md` §4.3 + §4.8 / `handoff-matrix.md` / `issue-lifecycle.md` 부분 read (grep + offset/limit). 통째 read 폐기 — 메인 cache_read 기준치 감축.
...
> **Hybrid A 분기 (#446 Step 4)** — 정식 위치 impl 파일 (`docs/milestones/v\d+/epics/epic-\d+-*/impl/\d+-*.md`) 존재 시 build-worker 2-step. 부재 시 후보 경로 (4-agent + module-architect 선두). 자세히 = [`orchestration.md §4.3 진입 모드`](../docs/plugin/orchestration.md). `/impl` 단발 호출은 본 Hybrid A 분기 미적용 — 4-agent 모델 유지 (엄정성 우선 의도된 티어링).
...
2. **task 한 개씩** — 아래 a~e 를 task N 이 완전히 끝난 (PR 머지 + 이슈 close) 뒤에만 task N+1 진입. 한 번에 한 task.
   ...
   2. **begin-run + build-worker step** — 메인이 `begin-run impl` 호출 → `begin-step build-worker` → `Agent(build-worker, prompt=<impl 경로 + task slug + RUN_ID>)` → 반환 prose 의 결론 분기:
      - `PASS` → step 3 진입
      - `SPEC_GAP_FOUND` → module-architect (보강) → build-worker 재호출 (cycle ≤ 2)
   ...
   3. **git/PR 생성 (메인)** — worker prose 의 commit message + PR 본문 초안 그대로 사용:
      ```bash
      git checkout -b <branch_prefix>/<task-slug>
      git add <changed files>
      git commit -m "<worker prose 의 commit message>"
      git push -u origin <branch>
      gh pr create --base <base> --title "<...>" --body "<worker prose 의 PR 본문>"
      ```
      `branch_prefix` = `orchestration.md §4.3 branch_prefix decision rule` 정합. base 분기 = `git-naming-spec.md §6` 정합 (통합 브랜치 모드 매치 시 그 값).
   ...
   5. **end-run + review 출력 재정의 (5줄 요약)** — `end-run` 발사. 메인 컨텍스트에 박는 echo 는 본 skill §"review 출력 재정의 (#446)" 의 5줄 요약. 원본 review.md 는 `<run_dir>/review.md` 디스크 그대로.
3. **enum 별 분기**:
   - `clean` → 다음 task 진입
...
### 후보 경로 (정식 위치 impl 부재 시)

정식 위치 impl 파일 부재 시 build-worker 미사용. 4-agent (test-engineer / engineer:IMPL / code-validator / pr-reviewer) + module-architect 선두 = 5-step 후보 경로. 이 경우 절차 §2.ii ~ §2.v 는 [`commands/impl.md`](impl.md) 본문 의무 그대로 (4-agent 시퀀스). 워크트리는 outer 1회.
...
후보 경로 (정식 위치 impl 부재) 시 sub-step 5건 (`module-architect` / `test-engineer` / `engineer` / `code-validator` / `pr-reviewer`).
...
- sub-step 수 = **2 (Hybrid A default)** / 5 (후보 경로: 정식 위치 impl 부재 시 module-architect 선두 + 4-agent)

**진입 시** — TaskCreate 를 이 순서로: `task1 헤더` → `task1 sub × 2 (Hybrid A) 또는 × 5 (후보 경로)` → `task2 헤더` → … → `taskN 헤더`. task1 헤더만 in_progress.

**task i 진행 중** — 그 task 의 `impl` run 은 TaskCreate skip (sub-step task 이미 존재 — loop-procedure §2). conveyor 진행에 맞춰 sub-step 을 TaskUpdate (pending → in_progress → completed). Hybrid A 경우 build-worker 안의 3 phase (build-test / build-impl / build-validate) 는 outer sub-step `build-worker` 하나로 묶임 — `activeForm` 에 phase 표시.
...
`/impl-loop` driver 안에서는 [`commands/impl.md`](impl.md) §종료 조건 의 review.md *원본 그대로 출력 MUST* 가 **driver 범위 안에서 재정의** 된다. 매 task 마다 review.md 전수를 메인 컨텍스트에 출력하면 N task = N × review.md 누적 → cache_read 폭주 (jajang 실측 기준선 ~280 turn/task 중 post-Agent 47 turn 의 주범 중 하나).
...
후보 경로 (정식 위치 impl 부재, 4-agent + module-architect) 일 때 2번째 줄은 `module-architect: <PASS|SPEC_GAP> · test-engineer: <N tests RED→GREEN> · engineer: <M files +X -Y> · code-validator: <PASS|FAIL> · pr-reviewer: <LGTM|FAIL>`.
...
**메인 인사이트** ([`commands/impl.md`](impl.md) §종료 조건 의 자율 1줄 인사이트) 는 본 요약 뒤에 *선택* 1줄 박을 수 있다 — 작성하면 누적 학습 신호, 생략해도 회귀 X.
...
## git 권한
worktree branch 안 commit / push / PR 생성·머지 = **메인 Claude 전담**. engineer / build-worker / test-engineer 등 sub-agent 는 git commit/push/branch + `gh pr create/merge` 금지 — 코드 변경만 (worker 의 경우 PR 본문·commit message *초안* 만 prose return 에 작성, 실제 명령은 메인). main 직접 commit / push = ❌ (main-block hook 차단). 상세 = [`git-naming-spec.md`](../docs/plugin/git-naming-spec.md) §6 + [`loop-procedure.md`](../docs/plugin/loop-procedure.md) §3.4 + [`agents/build-worker.md`](../agents/build-worker.md) §권한 경계.
...
## Hybrid A 모드 — 실측 진입 (#446 Step 4)

`/impl-loop` 메인 컨텍스트 누적 절감 목적의 Hybrid A 모드 활성 — 본 skill 의 §절차 가 build-worker 2-step 시퀀스를 기본으로 진행. 정식 위치 impl 파일 부재 시 후보 경로 (4-agent + module-architect) 자동 분기.
