...
**Base ref 분기 (MUST, #424)**: `docs/stories.md` 상단 `**Base Branch:** feature/<slug>` 마커 매치 시 = 통합 브랜치 모드. EnterWorktree default (`worktree.baseRef=fresh` = origin/main 기반) 가 sub-PR base 와 mismatch → 사전 `git worktree add -b <new> <path> origin/<integration>` + `EnterWorktree(path=<path>)` 패턴으로 outer worktree base 정합. 절차 = [`docs/plugin/loop-procedure.md`](../docs/plugin/loop-procedure.md) §1.1.1.
