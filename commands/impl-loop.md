...
`/impl-loop` driver 안에서는 [`commands/impl.md`](impl.md) §종료 조건 의 review.md *원본 그대로 출력 MUST* 가 **driver 범위 안에서 재정의** 된다. 매 task 마다 review.md 전수를 메인 컨텍스트에 출력하면 N task = N × review.md 누적 → cache_read 폭주 (jajang 실측 baseline ~280 turn/task 중 post-Agent 47 turn 의 주범 중 하나).
