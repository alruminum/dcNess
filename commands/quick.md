...
## 🔒 Step 기록 룰 (SSOT — DCN-30-25)

> **Agent 호출 1회 = `helper begin-step` + `helper end-step` 1쌍 의무**. POLISH 사이클 / 재호출 / git 작업 전후 모두 동일. skip 시 `.steps.jsonl` 누락 → `/run-review` 검출 안 됨 → 가시성 사단.
...
### POLISH / 재호출 시 step 이름 컨벤션

POLISH 사이클 (cycle ≤2) 의 step name:
- `engineer:POLISH-1` (첫 POLISH)
- `engineer:POLISH-2` (두 번째)

재호출 (TESTS_FAIL → engineer attempt 1+) step name:
- `engineer:IMPL-RETRY-1`, `engineer:IMPL-RETRY-2`

각각 별도 begin/end-step 1쌍.

### 안티패턴 — 실측 사례 기반 (jajang DCN-30-23 사후 분석)

❌ engineer 가 자체 commit/PR 만든 후 git status 확인 → end-step engineer 호출 *skip*
❌ pr-reviewer CHANGES_REQUESTED 받고 engineer POLISH Agent 호출하면서 begin/end-step 안 둘러쌈
❌ Agent 호출 후 사용자 입력 받느라 end-step 보류 → 다음 step 진입 시 망각
❌ multi-batch 진행 중 batch 간 보고 작성 후 begin-step 다시 안 부름

### 안전망 (helper 측 자동 검출 — DCN-30-25)

- **drift WARN**: `end-step` 호출 시 `live.json` 의 `current_step` 과 `args.agent` 불일치 → stderr WARN ("begin-step 누락 의심")
- **step count WARN**: `finalize-run --expected-steps N` 옵션. row count 미만 → stderr WARN ("inner step 누락")
- 자동 보정 X — 메인이 사후 인지 + `/run-review` 진단

skill 측 활용:
- `/impl` Step 7 finalize-run 시 `--expected-steps 5` 박음 (architect / test-engineer / engineer / validator / pr-reviewer)
- `/quick` Step 7 finalize-run 시 `--expected-steps 5` (qa / architect / engineer / validator / pr-reviewer)
