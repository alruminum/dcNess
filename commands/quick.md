...
**매 step 직후 — prose 요약 출력 (사용자 가시성)**:

```
[qa 분류] FUNCTIONAL_BUG
  대상: <파일:라인>
  근거: <prose 핵심 1줄>
```

(ctrl+o 로 agent 전체 prose 펼치기 가능)
...
**매 step 직후 prose 요약 출력**:

```
[architect LIGHT_PLAN] LIGHT_PLAN_READY
  계획: <한 줄 — 무엇을 어떻게 변경하는지>
  주요 결정: <빈 문자열 정책 등 핵심 결정 1줄>
```
...
[engineer IMPL] IMPL_DONE
  변경: <파일:라인> — <변경 요약 1줄>
  테스트: <engineer 가 돌린 테스트 결과 1줄, 없으면 skip>
```
...
```
[pr-reviewer] LGTM
  검토: <MUST FIX 0건 / NICE TO HAVE N건 1줄>
```
...
### Step 7 — run 종료 + commit/PR (clean 시 자동, caveat 시 확인)
...
다음 모두 충족 → "clean":
- Step 3 enum = `LIGHT_PLAN_READY`
- Step 4 enum = `IMPL_DONE`
- Step 5 enum = `PASS`
- Step 6 enum = `LGTM`
- 어느 step 도 `AMBIGUOUS` 아니었음
- pr-reviewer prose 안 `MUST FIX` 키워드 0건

조건 1개라도 미충족 → "caveat" → Step 7b (사용자 확인) 으로.

#### Step 7a — Clean 자동 진행

clean 인 경우 메인이 다음 순서 자동 실행 (사용자 확인 X):
...
2. **branch 생성** (분류별 prefix):
   - FUNCTIONAL_BUG → `fix/<짧은-슬러그>`
   - CLEANUP → `chore/<짧은-슬러그>`

   ```bash
   BRANCH="fix/<slug>"  # 또는 chore/
   git checkout -b "$BRANCH" main
   ```

3. **stage + commit** (modified tracked files 만 — `-A` 금지, 안전):
   ```bash
   git add $CHANGED
   git commit -m "$(cat <<'EOF'
   <한 줄 제목 — Step 0 의 이슈 제목 재사용>

   <본문 — engineer prose 의 핵심 변경 요약 + run_id 참조>
...
4. **remote 있으면 push + PR + squash merge**:
   ```bash
   ...
   EOF
     ...
     # squash merge — auto 가능하면 auto, 아니면 즉시
     gh pr merge --squash --auto 2>/dev/null || gh pr merge --squash || echo "[quick] PR 머지 차단 — branch protection 또는 CI 대기"
...
worktree 진입 (Step 0a) 했으면 PR merged 확인 후:
```
ExitWorktree(action="remove")  # auto-merge 성공 시
```

#### Step 7b — Caveat 확인 (clean 아닐 때)
...
⚠️ Caveat:
<감지된 caveat — 예: "pr-reviewer 가 MUST FIX 1건 지적", "validator FAIL", "AMBIGUOUS 1회">

커밋/PR 진행할까요? (글로벌 CLAUDE.md 절차 — branch → PR → squash merge)
```

사용자 응답 받으면 그제서야 git commit / push / gh pr create.

#### 자동 진행 차단 — 안전 가드

다음 상황에선 clean 이어도 자동 진행 차단 + 사용자 확인:

- `.gitignore` 안 있는 untracked 파일 (예: `.env`, `secrets.json` 같은 sensitive 후보)
- `git status` 가 unstaged + untracked 파일 다수 (10+ 개)
- 변경 파일 중 git submodule 안 또는 third-party vendor 디렉토리

이런 경우 메인이 caveat 으로 분류해 7b 로 진입.
...
- **commit/PR 자동화 (Step 7a)** — clean 조건 충족 시만. caveat (AMBIGUOUS / FAIL / MUST FIX / sensitive untracked 등) 1건이라도 있으면 사용자 확인 (7b) 으로 fallback.
- **doc-sync gate (governance §2.5)** — dcNess 자체 repo 가 아닌 사용자 프로젝트엔 적용 안 됨. dcNess 안에선 git pre-commit hook 이 차단할 수 있어 자동 commit 도 그 hook 통과해야 (정상 동작이면 통과).
