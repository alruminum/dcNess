...
## Loop
`qa-triage` ([orchestration.md §4.5](../docs/plugin/orchestration.md) / [loop-procedure.md §7](../docs/plugin/loop-procedure.md) Loop 카탈로그 cross-ref).
...
정상 흐름은 본 skill 본문 + 인용된 docs §번호 만으로 진행. *룰 모호 / 분기 발생* 시에만 `docs/plugin/orchestration.md` §4.5 (qa-triage 풀스펙) / `handoff-matrix.md §1.0` (routing 한눈표 — qa 행) 부분 read (grep + offset/limit). 통째 read 폐기 — 메인 cache_read 기준치 감축.

## 절차
[`docs/plugin/loop-procedure.md`](../docs/plugin/loop-procedure.md) §1~§6 (Step mechanics) + [`docs/plugin/orchestration.md`](../docs/plugin/orchestration.md) §4.5 (`qa-triage` 풀스펙) 따름. 본 파일은 input 명세 + 라우팅 추천만.
