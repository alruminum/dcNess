---
name: product-plan
description: 새 기능 / PRD 변경 / 큰 기획을 받아 메인 Claude 가 사용자와 직접 그릴미 대화하며 `docs/prd.md` + `docs/stories.md` + `docs/tech-review.md` *스켈레톤* 작성 → 사용자 1차 OK → PR 머지 + 이슈 등록 → `/tech-review` (선행 기술 검증) 권고 → 사용자 2차 OK 후 `/architect-loop` 진입 시퀀스로 진행하는 스킬. 사용자가 "기획자야", "새 기능", "피쳐 추가", "이런 기능이 필요할 것 같아", "기획해줘", "프로덕트 플랜", "/product-plan" 등을 말할 때 반드시 이 스킬을 사용한다. 구현 진입은 별도 (`/impl` / `/impl-loop`).
---
...
정상 흐름은 본 skill 본문 + 인용된 docs §번호 만으로 진행. *룰 모호 / 분기 발생* 시에만 `docs/plugin/loop-procedure.md` / `routing.md` §1 (라우팅) / `skills/architect-loop/SKILL.md` / `issue-lifecycle.md` 부분 read (grep + offset/limit). 통째 read 폐기 — 메인 cache_read 기준치 감축.
...
```
1. git checkout -b docs/<slug> main          # git-spec §1 패턴
2. git add docs/prd.md docs/stories.md
3. git commit -m "[docs] PRD 신규 / 변경 요약"   # git-spec §3 패턴
4. git push -u origin docs/<slug>
5. gh pr create --base main --title "..." --body "..."   # git-spec §4~§5
...
- 외부 의존 0 개인 PRD 케이스 → tech-review.md 스켈레톤이 "외부 의존 없음 — `/tech-review` skip" 상태 → 본 step 도 skip + 바로 `/architect-loop` 권고 echo
- 사용자 Y → `/tech-review` 진입
- `/tech-review` 종료 + 사용자 2 차 OK 후 → 메인이 `/architect-loop` 권고 echo (별도 명시 호출)

자세히 = [`commands/tech-review.md`](tech-review.md).
...
- PRD/stories/tech-review 스켈레톤 완성 + 머지 + 이슈 등록 후 → `/tech-review` (선행 기술 검증) → 사용자 2 차 OK → `/architect-loop` (설계 루프) → `/impl-loop` / `/impl` (구현 루프)
...
- 선행 기술 검증 (후속 스킬): [`commands/tech-review.md`](tech-review.md), [`agents/tech-reviewer.md`](../agents/tech-reviewer.md)
