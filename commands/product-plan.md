...
PRD 작성 완료 후 메인이 `docs/stories.md` Write — epic + N story 구조:

```markdown
# Story Backlog

**GitHub Epic Issue:** (이슈 등록 후 박힘 — PR2 시점)
...
스토리 본문 = `as a / i want / so that` + 수용 기준만. task-level 세부 진행 추적 X (PR + GitHub Issue close 시스템 SSOT — issue-lifecycle.md §0).
